...
<details>
  <summary>Details</summary>
Motivation: 现有偏见评估数据集主要针对英语和北美文化，且任务单一，无法全面评估LLM的偏见，尤其是在中文文化背景下。
...
TL;DR: 该论文探讨了在对话摘要任务中，逐步推理架构（如Long Chain-of-Thought）是否优于非推理架构，发现推理模型在摘要质量上并未一致提升，反而可能产生冗长和不一致的问题。
...
Result: 研究发现，逐步推理并未显著提升摘要质量，推理模型易产生冗长和不一致的问题。
...
TL;DR: 研究了Huginn-3.5B模型中是否存在隐式链式思维推理（latent CoT），发现其内部行为在算术任务中表现有限，且隐式推理的可解释性依赖于层索引和解码方法。
...
Result: 发现隐式CoT的证据有限，且不同循环块间的探测结果不一致；增加循环深度仅带来边际收益。

Conclusion: 隐式推理的效率提升有限，显式外部化推理步骤的模型表现更优。
...
Result: GDC Cohort Copilot能自动生成用户输入的自然语言描述对应的GDC队列过滤器，并通过交互界面进一步优化。

Conclusion: GDC Cohort Copilot是一个有效的工具，帮助用户更高效地创建和分析GDC队列。
...
TL;DR: MemAgent通过分段读取和覆盖更新内存的策略，优化长文本处理任务，在32K文本训练后能扩展到3.5M QA任务，性能损失小于5%。


<details>
  <summary>Details</summary>
Motivation: 解决长文本处理中线性复杂度下性能不下降的挑战。

Method: 引入MemAgent工作流，分段读取文本并覆盖更新内存，扩展DAPO算法以支持多对话生成训练。

Result: 在8K上下文训练后扩展到3.5M QA任务，性能损失<5%，512K RULER测试中达到95%+。
...
TL;DR: DoMIX提出了一种基于LoRA模块的新型持续领域自适应预训练方法，解决了现有方法的高计算成本、数据顺序敏感性和单一模型泛化问题。
...
Result: DoMIX不仅适用于领域自适应预训练，还可扩展到标准LLM微调场景。
...
TL;DR: 本文介绍了用于SciVQA 2025共享任务的系统，结合多模态大语言模型和少样本检索策略，根据图像和问题类型选择模型和设置，最终在盲测中排名第三。
...
TL;DR: 研究评估了七种基于Transformer架构的Akan ASR模型在不同领域的表现，发现模型性能受领域依赖性影响，并比较了Whisper和Wav2Vec2的错误行为差异。


<details>
  <summary>Details</summary>
Motivation: 现有ASR研究多基于同领域数据集，缺乏对跨领域泛化能力的评估，本研究填补了这一空白。

Method: 使用四种Akan语音语料库（涵盖不同领域）对七种ASR模型进行基准测试，比较词错误率和字符错误率。

Result: 模型在训练领域表现最佳，跨领域时性能显著下降；Whisper和Wav2Vec2在错误行为上存在差异。

Conclusion: 低资源语言ASR需考虑领域适应技术和架构选择（如错误可读性与透明性的权衡）。
...
TL;DR: 本研究提出了一种为低资源语言（如阿坎语）的语音障碍者收集语音样本的方法，旨在通过社区驱动的数据收集和ASR模型构建，推动ASR技术的民主化。


<details>
  <summary>Details</summary>
Motivation: 为语音障碍者开发包容性ASR技术，尤其是在低资源语言中，解决数据稀缺问题。

Method: 开发了一套“最佳实践指南”和培训方法，用于社区驱动的数据收集和模型构建，并以阿坎语为例创建了首个开源语音障碍数据集。

Result: 成功构建了阿坎语的开源语音障碍数据集，并展示了通过微调开源ASR模型提升语音障碍识别的初步成果。

Conclusion: 通过社区参与和开源工具，能够为语音障碍者定制更包容的ASR技术，推动技术民主化。
...
Conclusion: WebSailor有效缩小了开源模型与专有代理在复杂任务中的能力差距。
...
TL;DR: 论文探讨了标签变异（LV）和人类标签变异（HLV）在监督学习中的重要性，提出了一个将HLV纳入主动学习（AL）循环的概念框架。
...
Method: 通过分解标签变异为信号（HLV）和噪声（标注错误），提出HLV感知的AL框架，并探讨LLM作为标注者的整合。

Result: 提出了一个概念框架，以更真实地反映标注复杂性。

Conclusion: HLV感知的AL框架为实际标注问题提供了更全面的解决方案。
...
TL;DR: MPF是一种新型的后训练对齐框架，用于缓解大型语言模型（LLM）的偏见，通过多视角生成与人类基准对齐。
...
Method: 基于SAGED管道，分解基准为可解释的视角组件，通过采样和平衡响应进行对齐。

Result: 在情感分布对齐中表现出色，KL散度小，校准误差减少，并能泛化到未见问题。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索性别与语境偏见的深层关联，特别是职业相关偏见，并开发可解释的评估工具。

Method: 引入GenderLexicon数据集和框架，通过评分量化语境偏见及其性别偏见，并在五个多样化数据集上验证。

Result: 模型能有效解释性别偏见，证实了职业刻板印象外的性别偏见存在。

Conclusion: 研究提供了可解释的性别偏见评估工具，扩展了对性别偏见的理解。
...
TL;DR: 论文探讨了利用LLMs辅助同行评审，特别是识别论文局限性的潜力，并提出了LimitGen基准来评估LLMs的能力。


<details>
  <summary>Details</summary>
Motivation: 同行评审是科研的核心，但日益增长的论文数量加剧了这一过程的挑战，LLMs的潜力尚未充分研究。

Method: 提出了LimitGen基准，包含合成数据集LimitGen-Syn和真实数据集LimitGen-Human，并通过文献检索增强LLMs识别局限性的能力。
...
### [44] [Measurement of the Granularity of Vowel Production Space By Just Producible Different (JPD) Limens](https://arxiv.org/abs/2507.02744)
*Peter Viechnicki*

Main category: cs.CL

TL;DR: 研究测量了人类在模仿元音时能可靠区分的听觉空间最小距离（JPD），发现其范围为14至51 mels，对语音产生理论和元音系统结构有启示。


<details>
  <summary>Details</summary>
Motivation: 探讨人类在元音产生中对听觉空间目标的控制精度，填补了现有研究中对这一精度未知的空白。
...
### [45] [Self-Correction Bench: Revealing and Addressing the Self-Correction Blind Spot in LLMs](https://arxiv.org/abs/2507.02778)
*Ken Tsui*

Main category: cs.CL

TL;DR: 研究发现大型语言模型（LLMs）存在自我修正盲点，无法纠正自身输出中的错误，但通过简单提示（如“Wait”）可显著改善。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在自我修正能力上的局限性，尤其是对自身输出错误的识别与纠正能力。

Method: 引入Self-Correction Bench框架，通过控制性错误注入测试14个模型，分析盲点率及其与训练数据的关系。

Result: 平均盲点率为64.5%，发现训练数据中缺乏错误修正序列是主要原因，但简单提示可减少盲点89.3%。

Conclusion: 当前LLMs存在关键局限性，但通过调整训练或提示策略可提升其可靠性和可信度。
...
### [46] [Is Reasoning All You Need? Probing Bias in the Age of Reasoning Language Models](https://arxiv.org/abs/2507.02799)
*Riccardo Cantini,Nicola Gabriele,Alessio Orsino,Domenico Talia*
...
Method: 利用CLEAR-Bias基准，通过LLM-as-a-judge自动评分和越狱技术评估RLMs的安全性。

Result: 具备推理能力的模型（如CoT提示或微调推理轨迹）比基础模型更容易引发偏见，推理可能增加刻板印象的传播。
...
### [47] [Multimodal Mathematical Reasoning with Diverse Solving Perspective](https://arxiv.org/abs/2507.02804)
*Wenhao Shi,Zhiqiang Hu,Yi Bin,Yang Yang,See-Kiong Ng,Heng Tao Shen*
...
### [48] [SynapseRoute: An Auto-Route Switching Framework on Dual-State Large Language Model](https://arxiv.org/abs/2507.02822)
*Wencheng Zhang,Shiqin Qiao,Lingjie Luo,Yinfeng Li,Chuanyang Zheng,Qian Xu,Meng Li,Yong Gui,Yijun He,Jianing Qiu,Jindong Hong,Jiankai Sun*
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型广泛应用，需平衡性能和成本。研究发现58%医疗问题无需高成本推理即可解决，表明动态路由可优化效率。

Method: 提出SynapseRoute框架，基于机器学习动态路由查询到高推理或低成本模式。
...
### [49] [Generalizing Verifiable Instruction Following](https://arxiv.org/abs/2507.02833)
*Valentina Pyatkin,Saumya Malik,Victoria Graf,Hamish Ivison,Shengyi Huang,Pradeep Dasigi,Nathan Lambert,Hannaneh Hajishirzi*
...
<details>
  <summary>Details</summary>
Motivation: 当前最强模型在满足用户添加的输出约束条件（如“仅回答是或否”）时表现不佳，且容易过拟合。研究旨在提升模型在未见过约束上的泛化能力。

Method: 引入IFBench基准评估58种新约束的泛化能力，设计约束验证模块，并采用基于可验证奖励的强化学习（RLVR）进行训练。

Result: RLVR方法显著提升了模型在精确指令遵循上的泛化能力。

Conclusion: IFBench和RLVR为提升模型在输出约束条件下的表现提供了有效工具和数据支持。
...
### [50] [LLM Hypnosis: Exploiting User Feedback for Unauthorized Knowledge Injection to All Users](https://arxiv.org/abs/2507.02850)
*Almog Hilel,Idan Shenfeld,Leshem Choshen,Jacob Andreas*

Main category: cs.CL

TL;DR: 论文揭示了语言模型（LM）在用户反馈训练中的漏洞，单个用户可通过提供提示和点赞/点踩反馈持续改变LM的知识和行为。攻击者通过操纵反馈信号，使模型在后续偏好调整中更倾向于输出恶意内容。


<details>
  <summary>Details</summary>
Motivation: 研究动机是揭示语言模型在用户反馈训练中的潜在安全漏洞，尤其是单个用户如何通过有限权限（如点赞/点踩）对模型行为产生持久影响。

Method: 攻击方法包括：1）让模型随机输出“中毒”或良性响应；2）通过点赞中毒响应或点踩良性响应操纵反馈信号；3）利用反馈信号在后续偏好调整中增加中毒响应的概率。

Result: 结果显示，攻击可实现：1）插入模型原本不具备的事实知识；2）修改代码生成模式以引入安全漏洞；3）注入虚假金融新闻。

Conclusion: 结论指出，该研究不仅揭示了偏好调整的新特性（即有限反馈数据可精细控制模型行为），还提出了一种针对用户反馈训练LM的新型攻击机制。
...
### [51] [MOTIF: Modular Thinking via Reinforcement Fine-tuning in LLMs](https://arxiv.org/abs/2507.02851)
*Purbesh Mitra,Sennur Ulukus*

Main category: cs.CL

TL;DR: 论文提出MOTIF方法，通过强化学习微调实现多轮模块化思考，突破LLM上下文限制，在GSM8K数据集上训练Qwen2.5-3B-Instruct模型，并在MATH500和AIME2024基准测试中分别提升3.8%和3.3%的准确率。


<details>
  <summary>Details</summary>
Motivation: 解决LLM上下文大小限制导致的推理瓶颈，通过模块化思考策略扩展推理能力。

Method: 提出MOTIF方法，利用强化学习微调生成多轮思考令牌，结合参数高效微调技术训练模型。
...
### [52] [Answer Matching Outperforms Multiple Choice for Language Model Evaluation](https://arxiv.org/abs/2507.02856)
*Nikhil Chandak,Shashwat Goel,Ameya Prabhu,Moritz Hardt,Jonas Geiping*

Main category: cs.CL

TL;DR: 论文指出传统选择题评测存在缺陷，提出生成式评测（答案匹配）作为替代方案，并验证其与人类评分高度一致。


<details>
  <summary>Details</summary>
Motivation: 传统选择题评测存在漏洞，可能无需看题即可作答，而生成式评测能更准确评估模型能力。

Method: 提出答案匹配方法：模型生成自由回答，再用另一模型判断是否与参考答案匹配。通过MMLU-Pro和GPQA-Diamond数据集验证。

Result: 答案匹配方法评分与人类评分高度一致，优于选择题评测和LLM直接评判。

Conclusion: 生成式评测（答案匹配）是更可靠的评估方法，建议逐步替代选择题评测。
...
### [53] [Mesh Silksong: Auto-Regressive Mesh Generation as Weaving Silk](https://arxiv.org/abs/2507.02477)
...
### [54] [HyperGaussians: High-Dimensional Gaussian Splatting for High-Fidelity Animatable Face Avatars](https://arxiv.org/abs/2507.02803)
...
### [55] [Large Language Models for Crash Detection in Video: A Survey of Methods, Datasets, and Challenges](https://arxiv.org/abs/2507.02074)
...
### [56] [LiteReality: Graphics-Ready 3D Scene Reconstruction from RGB-D Scans](https://arxiv.org/abs/2507.02861)
...
### [57] [Underwater Monocular Metric Depth Estimation: Real-World Benchmarks and Synthetic Fine-Tuning](https://arxiv.org/abs/2507.02148)
...
### [58] [ESTR-CoT: Towards Explainable and Accurate Event Stream based Scene Text Recognition with Chain-of-Thought Reasoning](https://arxiv.org/abs/2507.02200)
...
### [59] [Team RAS in 9th ABAW Competition: Multimodal Compound Expression Recognition Approach](https://arxiv.org/abs/2507.02205)
...
### [60] [SciGA: A Comprehensive Dataset for Designing Graphical Abstracts in Academic Papers](https://arxiv.org/abs/2507.02212)
...
### [61] [Understanding Trade offs When Conditioning Synthetic Data](https://arxiv.org/abs/2507.02217)
...
### [62] [High-Fidelity Differential-information Driven Binary Vision Transformer](https://arxiv.org/abs/2507.02222)
...
### [63] [FMOcc: TPV-Driven Flow Matching for 3D Occupancy Prediction with Selective State Space Model](https://arxiv.org/abs/2507.02250)
...
### [64] [SurgVisAgent: Multimodal Agentic Model for Versatile Surgical Visual Enhancement](https://arxiv.org/abs/2507.02252)
...
### [65] [Multi-Label Classification Framework for Hurricane Damage Assessment](https://arxiv.org/abs/2507.02265)
...
### [66] [Cross-domain Hyperspectral Image Classification based on Bi-directional Domain Adaptation](https://arxiv.org/abs/2507.02268)
...
### [67] [MAC-Lookup: Multi-Axis Conditional Lookup Model for Underwater Image Enhancement](https://arxiv.org/abs/2507.02270)
...
### [68] [Spotlighting Partially Visible Cinematic Language for Video-to-Audio Generation via Self-distillation](https://arxiv.org/abs/2507.02271)
...
### [69] [LaCo: Efficient Layer-wise Compression of Visual Tokens for Multimodal Large Language Models](https://arxiv.org/abs/2507.02279)
...
### [70] [Prompt Disentanglement via Language Guidance and Representation Alignment for Domain Generalization](https://arxiv.org/abs/2507.02288)
...
### [71] [ViRefSAM: Visual Reference-Guided Segment Anything Model for Remote Sensing Segmentation](https://arxiv.org/abs/2507.02294)
...
### [72] [DreamComposer++: Empowering Diffusion Models with Multi-View Conditions for 3D Content Generation](https://arxiv.org/abs/2507.02299)
...
### [73] [Flow-CDNet: A Novel Network for Detecting Both Slow and Fast Changes in Bitemporal Images](https://arxiv.org/abs/2507.02307)
...
### [74] [LMPNet for Weakly-supervised Keypoint Discovery](https://arxiv.org/abs/2507.02308)
...
### [75] [Perception Activator: An intuitive and portable framework for brain cognitive exploration](https://arxiv.org/abs/2507.02311)
...
### [76] [MAGIC: Mask-Guided Diffusion Inpainting with Multi-Level Perturbations and Context-Aware Alignment for Few-Shot Anomaly Generation](https://arxiv.org/abs/2507.02314)
...
### [77] [Are Synthetic Videos Useful? A Benchmark for Retrieval-Centric Evaluation of Synthetic Videos](https://arxiv.org/abs/2507.02316)
...
### [78] [Heeding the Inner Voice: Aligning ControlNet Training via Intermediate Features Feedback](https://arxiv.org/abs/2507.02321)
...
### [79] [Neural Network-based Study for Rice Leaf Disease Recognition and Classification: A Comparative Analysis Between Feature-based Model and Direct Imaging Model](https://arxiv.org/abs/2507.02322)
...
### [80] [Two-Steps Neural Networks for an Automated Cerebrovascular Landmark Detection](https://arxiv.org/abs/2507.02349)
...
### [81] [Lightweight Shrimp Disease Detection Research Based on YOLOv8n](https://arxiv.org/abs/2507.02354)
...
### [82] [Holistic Tokenizer for Autoregressive Image Generation](https://arxiv.org/abs/2507.02358)
...
### [83] [LocalDyGS: Multi-view Global Dynamic Scene Modeling via Adaptive Local Implicit Feature Decoupling](https://arxiv.org/abs/2507.02363)
...
### [84] [UVLM: Benchmarking Video Language Model for Underwater World Understanding](https://arxiv.org/abs/2507.02373)
...
### [85] [PLOT: Pseudo-Labeling via Video Object Tracking for Scalable Monocular 3D Object Detection](https://arxiv.org/abs/2507.02393)
...
### [86] [Continual Multiple Instance Learning with Enhanced Localization for Histopathological Whole Slide Image Analysis](https://arxiv.org/abs/2507.02395)
...
### [87] [From Long Videos to Engaging Clips: A Human-Inspired Video Editing Framework with Multimodal Narrative Understanding](https://arxiv.org/abs/2507.02790)
...
### [88] [Beyond Spatial Frequency: Pixel-wise Temporal Frequency-based Deepfake Video Detection](https://arxiv.org/abs/2507.02398)
...
### [89] [TABNet: A Triplet Augmentation Self-Recovery Framework with Boundary-Aware Pseudo-Labels for Medical Image Segmentation](https://arxiv.org/abs/2507.02399)
...
### [90] [Visual Contextual Attack: Jailbreaking MLLMs with Image-Driven Context Injection](https://arxiv.org/abs/2507.02844)
...
### [94] [Privacy-preserving Preselection for Face Identification Based on Packing](https://arxiv.org/abs/2507.02414)
...
### [95] [Determination Of Structural Cracks Using Deep Learning Frameworks](https://arxiv.org/abs/2507.02416)
...
### [96] [AvatarMakeup: Realistic Makeup Transfer for 3D Animatable Head Avatars](https://arxiv.org/abs/2507.02419)
...
### [97] [F^2TTA: Free-Form Test-Time Adaptation on Cross-Domain Medical Image Classification via Image-Level Disentangled Prompt Tuning](https://arxiv.org/abs/2507.02437)
...
### [100] [Weakly-supervised Contrastive Learning with Quantity Prompts for Moving Infrared Small Target Detection](https://arxiv.org/abs/2507.02454)
...
### [102] [MedFormer: Hierarchical Medical Vision Transformer with Content-Aware Dual Sparse Selection Attention](https://arxiv.org/abs/2507.02488)
...
### [103] [Temporally-Aware Supervised Contrastive Learning for Polyp Counting in Colonoscopy](https://arxiv.org/abs/2507.02493)
...
### [104] [MC-INR: Efficient Encoding of Multivariate Scientific Simulation Data using Meta-Learning and Clustered Implicit Neural Representations](https://arxiv.org/abs/2507.02494)
...
### [105] [Detecting Multiple Diseases in Multiple Crops Using Deep Learning](https://arxiv.org/abs/2507.02517)
...
### [106] [Automatic Labelling for Low-Light Pedestrian Detection](https://arxiv.org/abs/2507.02513)
...
### [107] [IMASHRIMP: Automatic White Shrimp (Penaeus vannamei) Biometrical Analysis from Laboratory Images Using Computer Vision and Deep Learning](https://arxiv.org/abs/2507.02519)
...
### [108] [Addressing Camera Sensors Faults in Vision-Based Navigation: Simulation and Dataset Development](https://arxiv.org/abs/2507.02602)
...
### [109] [MoGe-2: Accurate Monocular Geometry with Metric Scale and Sharp Details](https://arxiv.org/abs/2507.02546)
...
### [110] [APT: Adaptive Personalized Training for Diffusion Models with Limited Data](https://arxiv.org/abs/2507.02687)
...
### [111] [Reconstructing Close Human Interaction with Appearance and Proxemics Reasoning](https://arxiv.org/abs/2507.02565)
...
### [112] [FairHuman: Boosting Hand and Face Quality in Human Image Generation with Minimum Potential Delay Fairness in Diffusion Models](https://arxiv.org/abs/2507.02714)
...
### [114] [Linear Attention with Global Context: A Multipole Attention Mechanism for Vision and Physics](https://arxiv.org/abs/2507.02748)
...
### [126] [Partial Weakly-Supervised Oriented Object Detection](https://arxiv.org/abs/2507.02751)
...
### [127] [From Pixels to Damage Severity: Estimating Earthquake Impacts Using Semantic Segmentation of Social Media Images](https://arxiv.org/abs/2507.02781)
...
### [128] [RichControl: Structure- and Appearance-Rich Training-Free Spatial Control for Text-to-Image Generation](https://arxiv.org/abs/2507.02792)
...
### [129] [No time to train! Training-Free Reference-Based Instance Segmentation](https://arxiv.org/abs/2507.02798)
...
### [130] [LangScene-X: Reconstruct Generalizable 3D Language-Embedded Scenes with TriMap Video Diffusion](https://arxiv.org/abs/2507.02813)
...
### [131] [Confidence-driven Gradient Modulation for Multimodal Human Activity Recognition: A Dynamic Contrastive Dual-Path Learning Approach](https://arxiv.org/abs/2507.02826)
...
### [132] [AnyI2V: Animating Any Conditional Image with Motion Control](https://arxiv.org/abs/2507.02857)
...
### [133] [Bootstrapping Grounded Chain-of-Thought in Multimodal LLMs for Data-Efficient Model Adaptation](https://arxiv.org/abs/2507.02859)
...
### [134] [Less is Enough: Training-Free Video Diffusion Acceleration via Runtime-Adaptive Caching](https://arxiv.org/abs/2507.02860)
...
### [135] [RefTok: Reference-Based Tokenization for Video Generation](https://arxiv.org/abs/2507.02862)
...
### [136] [Mesh Silksong: Auto-Regressive Mesh Generation as Weaving Silk](https://arxiv.org/abs/2507.02477)
*Gaochao Song,Zibo Zhao,Haohan Weng,Jingbo Zeng,Rongfei Jia,Shenghua Gao*
...
Result: 生成的网格具有优异的几何特性（如流形拓扑、水密检测和一致的法线），实验证明了其高效性和几何完整性。

Conclusion: Mesh Silksong在网格生成和几何特性上表现优异，适用于实际应用。
...
### [137] [HyperGaussians: High-Dimensional Gaussian Splatting for High-Fidelity Animatable Face Avatars](https://arxiv.org/abs/2507.02803)
*Gent Serifi,Marcel C. Bühler*

Main category: cs.CV

TL;DR: HyperGaussians是一种基于3D高斯泼溅的新方法，用于高质量可动画化面部虚拟形象，解决了现有方法在非线性变形和复杂光照下的不足。


<details>
  <summary>Details</summary>
Motivation: 现有的3D高斯泼溅方法在静态面部表现良好，但在可动画化虚拟形象中仍存在细节不足的问题。

Method: 通过将3D高斯扩展到高维多变量高斯（HyperGaussians），并结合可学习的局部嵌入和逆协方差技巧提升表达能力与效率。
...
### [138] [Large Language Models for Crash Detection in Video: A Survey of Methods, Datasets, and Challenges](https://arxiv.org/abs/2507.02074)
*Sanjeda Akter,Ibne Farabi Shihab,Anuj Sharma*

Main category: cs.CV

TL;DR: 本文综述了利用大语言模型（LLMs）从视频数据中检测碰撞的方法，包括融合策略分类、数据集总结、模型架构分析和性能比较，并讨论了挑战与机遇。
...
### [139] [LiteReality: Graphics-Ready 3D Scene Reconstruction from RGB-D Scans](https://arxiv.org/abs/2507.02861)
*Zhening Huang,Xiaoyang Wu,Fangcheng Zhong,Hengshuang Zhao,Matthias Nießner,Joan Lasenby*
...
Result: 在Scan2CAD基准测试中达到最先进的相似性性能，并能处理复杂的材质和光照问题。
...
### [140] [Underwater Monocular Metric Depth Estimation: Real-World Benchmarks and Synthetic Fine-Tuning](https://arxiv.org/abs/2507.02148)
*Zijie Cai,Christopher Metzler*

Main category: cs.CV

TL;DR: 论文评估了单目深度估计模型在水下环境中的性能，发现传统模型因域偏移表现不佳，通过微调Depth Anything V2模型在合成水下数据上，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 水下环境的光衰减、散射、颜色失真和缺乏高质量真实数据限制了单目深度估计的可靠性，因此需要研究其在水下的表现和改进方法。

Method: 使用FLSea和SQUID等真实水下数据集评估零样本和微调模型，并基于合成水下数据微调Depth Anything V2模型。

Result: 传统模型在水下表现不佳，微调后的模型在所有基准测试中均优于仅使用清洁空中数据的基线模型。

Conclusion: 研究强调了域适应和尺度感知监督对水下环境中鲁棒深度预测的重要性，为未来研究提供了方向。
...
### [141] [ESTR-CoT: Towards Explainable and Accurate Event Stream based Scene Text Recognition with Chain-of-Thought Reasoning](https://arxiv.org/abs/2507.02200)
*Xiao Wang,Jingtao Jiang,Qiang Chen,Lan Chen,Lin Zhu,Yaowei Wang,Yonghong Tian,Jin Tang*
...
<details>
  <summary>Details</summary>
Motivation: 现有的事件流场景文本识别方法在极端场景（如低光照、快速运动）中表现优于RGB相机，但仍受限于解释性不足和上下文逻辑推理能力弱的问题。

Method: 采用EVA-CLIP视觉编码器将事件流转换为token，结合Llama tokenizer和Q-former对齐预训练大语言模型Vicuna-7B，输出答案和推理过程。通过监督微调端到端优化框架，并构建大规模CoT数据集进行训练。

Result: 在三个事件流STR基准数据集（EventSTR、WordArt*、IC15*）上的实验验证了框架的有效性和解释性。

Conclusion: ESTR-CoT框架在事件流场景文本识别中表现出色，兼具解释性和逻辑推理能力，为后续推理大模型的发展提供了数据基础。
...
### [142] [Team RAS in 9th ABAW Competition: Multimodal Compound Expression Recognition Approach](https://arxiv.org/abs/2507.02205)
*Elena Ryumina,Maxim Markitantov,Alexandr Axyonov,Dmitry Ryumin,Mikhail Dolgushin,Alexey Karpov*

Main category: cs.CV

TL;DR: 提出了一种零样本多模态方法用于复合表情识别（CER），结合六种模态，并引入动态权重融合模块和复合表情转换模块，在多个数据集上表现接近监督方法。


<details>
  <summary>Details</summary>
Motivation: 传统CER方法依赖特定任务训练数据，本文旨在通过零样本方法捕捉复合情绪，避免领域适应。

Method: 结合六种模态（静态/动态面部表情、场景/标签匹配、场景上下文、音频、文本），使用CLIP和Qwen-VL进行零样本处理，引入MHPF模块动态加权预测，CE转换模块通过PPA和PFSA生成可解释输出。

Result: 在AffWild2、AFEW和C-EXPR-DB数据集上的F1分数分别为46.95%、49.02%和34.85%，表现接近监督方法。

Conclusion: 提出的零样本方法有效捕捉复合情绪，无需领域适应，代码已公开。
...
### [143] [SciGA: A Comprehensive Dataset for Designing Graphical Abstracts in Academic Papers](https://arxiv.org/abs/2507.02212)
*Takuro Kawada,Shunsuke Kitada,Sota Nemoto,Hitoshi Iyatomi*

Main category: cs.CV

TL;DR: SciGA-145k是一个大规模数据集，用于支持图形摘要（GA）的选择、推荐和自动生成研究，提出了两种任务和一种新评估指标CAR。
...
Method: 引入SciGA-145k数据集，定义两种任务（Intra-GA和Inter-GA推荐）并提出CAR评估指标。

Result: 提供了基线模型和新评估指标CAR，为GA设计和AI研究奠定基础。
...
### [144] [Understanding Trade offs When Conditioning Synthetic Data](https://arxiv.org/abs/2507.02217)
*Brandon Trabucco,Qasim Wani,Benjamin Pikus,Vasu Sharma*
...
<details>
  <summary>Details</summary>
Motivation: 工业视觉系统中高质量训练数据收集耗时，合成数据成为关键解决方案，但现有方法生成速度慢且仿真与现实差距大。扩散模型虽能快速生成高质量图像，但在低数据量下精确控制仍困难。

Method: 研究从四个标准物体检测基准中选取80个视觉概念，比较基于提示和基于布局的两种条件策略。

Result: 当条件线索多样时，布局条件策略优于提示条件策略；若布局线索匹配完整训练分布，合成数据使平均精度提升34%，最高达177%。

Conclusion: 布局条件策略在多样化条件下表现更优，能显著提升物体检测性能。
...
### [145] [High-Fidelity Differential-information Driven Binary Vision Transformer](https://arxiv.org/abs/2507.02222)
*Tian Gao,Zhiyuan Zhang,Kaijie Yin,Xu-Cheng Zhong,Hui Kong*

Main category: cs.CV

TL;DR: DIDB-ViT是一种新型的二值化视觉变换器，通过差分信息增强注意力模块和高频保留，解决了现有二值化ViT方法的性能下降问题。
...
Method: 设计差分信息注意力模块，使用离散Haar小波进行频率分解，改进RPReLU激活函数。
...
### [146] [FMOcc: TPV-Driven Flow Matching for 3D Occupancy Prediction with Selective State Space Model](https://arxiv.org/abs/2507.02250)
*Jiangxia Chen,Tongyuan Huang,Ke Song*
...
### [147] [SurgVisAgent: Multimodal Agentic Model for Versatile Surgical Visual Enhancement](https://arxiv.org/abs/2507.02252)
*Zeyu Lei,Hongyuan Yu,Jinlin Wu,Zhen Chen*

Main category: cs.CV

TL;DR: 提出了一种基于多模态大语言模型（MLLMs）的智能手术视觉代理SurgVisAgent，能够动态识别内窥镜图像的失真类别和严重程度，并执行多种增强任务。
...
### [148] [Multi-Label Classification Framework for Hurricane Damage Assessment](https://arxiv.org/abs/2507.02265)
*Zhangding Liu,Neda Mohammadi,John E. Taylor*
...
### [149] [Cross-domain Hyperspectral Image Classification based on Bi-directional Domain Adaptation](https://arxiv.org/abs/2507.02268)
*Yuxiang Zhang,Wei Li,Wen Jia,Mengmeng Zhang,Ran Tao,Shunlin Liang*
...
Result: 在跨时空数据集上表现优于现有域适应方法，在树种分类任务中比最先进方法高3%～5%。
...
### [150] [MAC-Lookup: Multi-Axis Conditional Lookup Model for Underwater Image Enhancement](https://arxiv.org/abs/2507.02270)
*Fanghai Yi,Zehong Zheng,Zexiao Liang,Yihang Dong,Xiyang Fang,Wangyu Wu,Xuhang Chen*
...
<details>
  <summary>Details</summary>
Motivation: 水下图像因光线变化、水体浑浊和气泡等问题存在可见性和颜色问题，传统方法效果不佳，而深度学习缺乏高质量数据集。

Method: 模型包含条件3D查找表颜色校正（CLTCC）和多轴自适应增强（MAAE）模块，分别用于初步颜色校正和细节优化。
...
### [151] [Spotlighting Partially Visible Cinematic Language for Video-to-Audio Generation via Self-distillation](https://arxiv.org/abs/2507.02271)
*Feizhen Huang,Yu Wu,Yutian Lin,Bo Du*
...
### [152] [LaCo: Efficient Layer-wise Compression of Visual Tokens for Multimodal Large Language Models](https://arxiv.org/abs/2507.02279)
*Juntao Liu,Liqiang Niu,Wenchao Chen,Jie Zhou,Fandong Meng*
...
Method: LaCo引入层级的像素重组机制和残差学习架构，通过空间到通道的转换合并相邻令牌。
...
TL;DR: 本文提出了一种基于视觉基础模型（VFM）的文本特征引导视觉提示调优框架，结合语言模型和视觉提示，通过WERA方法增强域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决域泛化（DG）中如何设计能够解耦跨域不变特征的提示这一关键挑战。

Method: 利用VFM的可控语言提示，通过LLM自动解耦文本提示，并引入WERA方法扩展视觉提示，结合风格化图像增强和一致性约束。

Result: 在多个DG数据集上表现优于现有方法。

Conclusion: 提出的框架通过文本和视觉特征的协同解耦，显著提升了域泛化性能。
...
### [154] [ViRefSAM: Visual Reference-Guided Segment Anything Model for Remote Sensing Segmentation](https://arxiv.org/abs/2507.02294)
*Hanbo Bi,Yulong Xu,Ya Li,Yongqiang Mao,Boyuan Tong,Chongyang Li,Chunbo Lang,Wenhui Diao,Hongqi Wang,Yingchao Feng,Xian Sun*
...
Conclusion: ViRefSAM通过少量参考图像实现了高效、自动的遥感图像分割。
...
### [155] [DreamComposer++: Empowering Diffusion Models with Multi-View Conditions for 3D Content Generation](https://arxiv.org/abs/2507.02299)
*Yunhan Yang,Shuo Chen,Yukun Huang,Xiaoyang Wu,Yuan-Chen Guo,Edmund Y. Lam,Hengshuang Zhao,Tong He,Xihui Liu*

Main category: cs.CV

TL;DR: DreamComposer++ 是一个改进多视角条件控制的框架，通过提取和融合多视角的3D表示，提升现有扩散模型生成可控新视角的能力。
...
Method: 利用视角感知的3D提升模块提取多视角的3D表示，通过多视角特征融合模块将其渲染为目标视角的潜在特征，并集成到预训练的扩散模型中。

Result: 实验表明，DreamComposer++ 能无缝集成先进模型，显著提升多视角条件下的可控新视角生成能力。

Conclusion: 该框架为可控3D对象重建和广泛应用提供了新可能。
...
### [156] [Flow-CDNet: A Novel Network for Detecting Both Slow and Fast Changes in Bitemporal Images](https://arxiv.org/abs/2507.02307)
*Haoxuan Li,Chenxu Wei,Haodong Wang,Xiaomeng Hu,Boyuan An,Lingyan Ran,Baosen Zhang,Jin Jin,Omirzhan Taukebayev,Amirkhan Temirbayev,Junrui Liu,Xiuwei Zhang*

Main category: cs.CV

TL;DR: 提出了一种名为Flow-CDNet的双分支网络，用于同时检测慢速和快速变化，结合光流和二进制变化检测分支，并在自建数据集Flow-Change上验证了其优越性。
...
Method: Flow-CDNet包含光流分支和二进制变化检测分支，前者通过金字塔结构提取多尺度位移变化，后者结合ResNet和光流分支输出生成快速变化结果。

Result: 在自建数据集Flow-Change上的定量实验表明，该方法优于现有方法；消融实验验证了双分支的相互促进作用。

Conclusion: Flow-CDNet能够有效检测慢速和快速变化，双分支设计提升了检测性能，为实际应用提供了新思路。
...
### [157] [LMPNet for Weakly-supervised Keypoint Discovery](https://arxiv.org/abs/2507.02308)
*Pei Guo,Ryan Farrell*
...
<details>
  <summary>Details</summary>
Motivation: 研究目标是通过仅使用类别标签的弱监督方式，发现语义对象关键点，避免依赖手工设计的损失项。

Method: 提出Leaky Max Pooling (LMP)层和选择策略，结合注意力掩码和可学习聚类层，实现关键点检测。

Result: LMPNet能自动发现与物体姿态无关的语义关键点，预测精度接近有监督的姿态估计模型。

Conclusion: LMPNet通过直接操作网络滤波器检测预定义概念，具有高度可解释性，并在弱监督下表现优异。
...
### [158] [Perception Activator: An intuitive and portable framework for brain cognitive exploration](https://arxiv.org/abs/2507.02311)
*Le Xu,Qi Zhang,Qixian Zhang,Hongyun Zhang,Duoqian Miao,Cairong Zhao*

Main category: cs.CV

TL;DR: 论文提出了一种利用fMRI信号增强视觉任务性能的实验框架，通过跨注意力机制将fMRI表征注入多尺度图像特征，验证了fMRI信号对下游检测和分割任务的提升作用。


<details>
  <summary>Details</summary>
Motivation: 现有的大脑视觉解码方法主要依赖像素级对齐，缺乏细粒度语义对齐，导致重建结果存在明显失真。研究旨在探索fMRI信号中丰富的多对象语义线索和空间定位信息。

Method: 开发了一个实验框架，通过跨注意力机制将fMRI表征注入多尺度图像特征，对比有无fMRI信息的下游任务性能和中间特征变化。

Result: 实验表明，引入fMRI信号提升了检测和分割任务的准确性，证实了fMRI信号包含多对象语义线索和粗粒度空间定位信息。

Conclusion: fMRI信号包含当前模型尚未充分利用的语义和空间信息，为未来大脑视觉解码研究提供了新方向。
...
### [159] [MAGIC: Mask-Guided Diffusion Inpainting with Multi-Level Perturbations and Context-Aware Alignment for Few-Shot Anomaly Generation](https://arxiv.org/abs/2507.02314)
*JaeHyuck Choi,MinJun Kim,JeHyeong Hong*
...
<details>
  <summary>Details</summary>
Motivation: 工业质量控制中异常数据稀缺，现有方法无法同时满足保持背景完整、精确填充异常区域和生成语义合理且多样化的异常。

Method: MAGIC基于Stable Diffusion修复框架，结合高斯提示级扰动和空间噪声注入，并引入上下文感知对齐模块。
...
### [160] [Are Synthetic Videos Useful? A Benchmark for Retrieval-Centric Evaluation of Synthetic Videos](https://arxiv.org/abs/2507.02316)
*Zecheng Zhao,Selena Song,Tong Chen,Zhi Chen,Shazia Sadiq,Yadan Luo*
...
Result: SynTVA不仅作为基准，还能选择高质量合成样本，显著提升TVR性能。

Conclusion: SynTVA为数据集增强和TVR任务提供了有价值的资源。
...
### [161] [Heeding the Inner Voice: Aligning ControlNet Training via Intermediate Features Feedback](https://arxiv.org/abs/2507.02321)
*Nina Konovalova,Maxim Nikolaev,Andrey Kuznetsov,Aibek Alanov*
...
<details>
  <summary>Details</summary>
Motivation: 尽管文本到图像扩散模型取得了显著进展，但在生成输出中实现精确的空间控制仍然具有挑战性。现有的方法（如ControlNet++）仅关注最终去噪步骤，忽略了中间生成阶段，限制了其效果。
...
### [162] [Neural Network-based Study for Rice Leaf Disease Recognition and Classification: A Comparative Analysis Between Feature-based Model and Direct Imaging Model](https://arxiv.org/abs/2507.02322)
*Farida Siddiqi Prity,Mirza Raquib,Saydul Akbar Murad,Md. Jubayar Alam Rafi,Md. Khairul Bashar Bhuiyan,Anupam Kumar Bairagi*

Main category: cs.CV

TL;DR: 该研究提出了一种基于人工神经网络（ANN）的图像处理技术，用于水稻叶病的早期分类和识别，并通过实验比较了特征分析检测模型（FADM）和直接图像中心检测模型（DICDM），发现FADM性能更优。


<details>
  <summary>Details</summary>
Motivation: 水稻叶病严重影响产量和经济收益，早期检测对有效管理和提高产量至关重要。现有方法缺乏对FADM和DICDM的全面比较，尤其是特征提取算法（FEAs）的效果评估。

Method: 研究采用FADM，结合多种图像特征提取算法、降维算法、特征选择算法和极限学习机（ELM），并与不使用FEA的DICDM进行对比，使用10折交叉验证方法评估性能。
...
### [163] [Two-Steps Neural Networks for an Automated Cerebrovascular Landmark Detection](https://arxiv.org/abs/2507.02349)
*Rafic Nader,Vincent L'Allinec,Romain Bourcier,Florent Autrusseau*

Main category: cs.CV

TL;DR: 提出了一种全自动的两步神经网络方法，用于检测颅内动脉瘤（ICA）在Willis环（CoW）分叉处的关键标志点。
...
Result: 在两种脑MRA数据集上验证，该方法在分叉检测任务中表现最佳。

Conclusion: 该方法有效解决了标志点漏检和解剖变异问题，提升了检测性能。
...
### [164] [Lightweight Shrimp Disease Detection Research Based on YOLOv8n](https://arxiv.org/abs/2507.02354)
*Fei Yuhuan,Wang Gengchen,Liu Fenghao,Zang Ran,Sun Xufei,Chang Hao*
...
<details>
  <summary>Details</summary>
Motivation: 虾病是虾类养殖中经济损失的主要原因之一，亟需高效的智能检测方法以减少疾病传播。

Method: 设计了RLDD检测头和C2f-EMCM模块降低计算复杂度，并引入改进的SegNext_Attention自注意力机制增强特征提取能力。

Result: 模型参数减少32.3%，mAP@0.5达92.7%（比YOLOv8n提升3%），在URPC2020数据集上泛化能力更强。

Conclusion: 该方法在准确性和效率之间取得平衡，为虾类养殖的智能疾病检测提供了可靠技术支持。
...
Method: Hita采用整体到局部标记方案，结合可学习的整体查询和局部补丁标记，并引入顺序结构和轻量级融合模块优化信息流。

Result: Hita在ImageNet基准测试中达到2.59 FID和281.9 IS，训练速度更快，并在零样本风格迁移和图像修复中表现优异。

Conclusion: Hita通过捕捉全局图像属性显著提升自回归生成性能，具有广泛的应用潜力。
...
### [166] [LocalDyGS: Multi-view Global Dynamic Scene Modeling via Adaptive Local Implicit Feature Decoupling](https://arxiv.org/abs/2507.02363)
*Jiahao Wu,Rui Peng,Jianbo Jiao,Jiayu Yang,Luyang Tang,Kaiqiang Xiong,Jie Liang,Jinbo Yan,Runling Liu,Ronggang Wang*
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的复杂动态运动使得从多视角输入合成任意视角的动态视频具有挑战性。现有方法（如神经辐射场或3D高斯泼溅）难以建模大尺度运动。
...
### [167] [UVLM: Benchmarking Video Language Model for Underwater World Understanding](https://arxiv.org/abs/2507.02373)
*Xizhe Xue,Yang Zhou,Dawei Yan,Ying Li,Haokui Zhang,Rong Xiao*
...
Conclusion: UVLM为水下观测提供了有效的基准，展示了其在提升模型性能方面的潜力。
...
### [168] [PLOT: Pseudo-Labeling via Video Object Tracking for Scalable Monocular 3D Object Detection](https://arxiv.org/abs/2507.02393)
*Seokyeong Lee,Sithu Aung,Junyong Choi,Seungryong Kim,Ig-Jae Kim,Junghyun Cho*
...
Method: 提出伪标签框架，利用视频数据，通过对象点跟踪聚合静态和动态对象的伪LiDAR数据，无需多视角设置或额外传感器。
...
### [169] [Continual Multiple Instance Learning with Enhanced Localization for Histopathological Whole Slide Image Analysis](https://arxiv.org/abs/2507.02395)
*Byung Hyun Lee,Wongi Jeong,Woojae Han,Kyoungbun Lee,Se Young Chun*

Main category: cs.CV

TL;DR: CoMEL是一种用于持续多实例学习的框架，通过增强定位和减少遗忘，显著提升了WSI数据集的性能。


<details>
  <summary>Details</summary>
Motivation: 解决多实例学习（MIL）在持续任务中的适应性问题，特别是针对定位任务中的遗忘问题。

Method: 提出CoMEL框架，包括GDAT、BPPL和OWLoRA三个模块，分别用于实例编码、伪标签生成和遗忘缓解。

Result: 在三个公开WSI数据集上，CoMEL在袋级和定位准确率上分别提升了11.00%和23.4%。

Conclusion: CoMEL在持续MIL任务中表现出色，为WSI的定位和分类提供了高效解决方案。
...
### [170] [From Long Videos to Engaging Clips: A Human-Inspired Video Editing Framework with Multimodal Narrative Understanding](https://arxiv.org/abs/2507.02790)
*Xiangfeng Wang,Xiao Li,Yadong Wei,Xueyu Song,Yang Song,Xiaoqiang Xia,Fangrui Zeng,Zaiyi Chen,Liu Liu,Gu Xu,Tong Xu*

Main category: cs.CV

TL;DR: 提出了一种基于多模态叙事理解的自动视频编辑框架HIVE，通过角色提取、对话分析和叙事总结提升编辑质量，并在新数据集DramaAD上验证了其优越性。
...
### [171] [Beyond Spatial Frequency: Pixel-wise Temporal Frequency-based Deepfake Video Detection](https://arxiv.org/abs/2507.02398)
*Taehoon Kim,Jongwook Choi,Yonghyun Jeong,Haeun Noh,Jaejun Yoo,Seungryul Baek,Jongwon Choi*
...
### [172] [TABNet: A Triplet Augmentation Self-Recovery Framework with Boundary-Aware Pseudo-Labels for Medical Image Segmentation](https://arxiv.org/abs/2507.02399)
*Peilin Zhang,Shaouxan Wua,Jun Feng,Zhuo Jin,Zhizezhang Gao,Jingkun Chen,Yaqiong Xing,Xiao Zhang*
...
TL;DR: 该论文提出了一种新型的视觉中心越狱攻击（VisCo攻击），通过动态生成辅助图像和优化攻击提示，显著提高了对多模态大语言模型（MLLMs）的攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉模态攻击方法存在语义模糊和缺乏现实场景基础的问题，因此需要一种更有效的视觉中心越狱攻击方法。

Method: 提出VisCo攻击，采用四种视觉策略构建上下文对话，并结合自动毒性模糊和语义优化生成攻击提示。

Result: VisCo在MM-SafetyBench上对GPT-4o的毒性得分为4.78，攻击成功率为85%，显著优于基线方法。

Conclusion: VisCo攻击展示了视觉信息在多模态模型安全漏洞中的关键作用，为未来防御研究提供了重要参考。
...
### [174] [Wildlife Target Re-Identification Using Self-supervised Learning in Non-Urban Settings](https://arxiv.org/abs/2507.02403)
*Mufhumudzi Muthivhi,Terence L. van Zyl*

Main category: cs.CV

TL;DR: 本文研究了自监督学习在野生动物重识别中的应用，通过无监督方式从相机陷阱数据中提取图像对训练模型，实验表明自监督模型在数据有限时更鲁棒，且在下游任务中表现优于监督学习。
...
### [175] [PosDiffAE: Position-aware Diffusion Auto-encoder For High-Resolution Brain Tissue Classification Incorporating Artifact Restoration](https://arxiv.org/abs/2507.02405)
*Ayantika Das,Moitreya Chaudhuri,Koushik Bhat,Keerthi Ram,Mihail Bota,Mohanasankar Sivaprakasam*

Main category: cs.CV

TL;DR: 该论文提出了一种结合扩散模型和自动编码器的方法，通过学习图像特定表示并组织潜在空间，用于脑图像分析和修复任务。
...
Conclusion: 结合扩散模型和自动编码器的方法在图像表示学习和修复任务中表现出色，为医学图像分析提供了新工具。
...
### [176] [A Novel Tuning Method for Real-time Multiple-Object Tracking Utilizing Thermal Sensor with Complexity Motion Pattern](https://arxiv.org/abs/2507.02408)
*Duong Nguyen-Ngoc Tran,Long Hoang Pham,Chi Dai Tran,Quoc Pham-Nam Ho,Huy-Hung Nguyen,Jae Wook Jeon*

Main category: cs.CV

TL;DR: 论文提出了一种针对热成像中行人跟踪的新型调优方法，通过优化两阶段的超参数，提高跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 热成像在低可见度或光线不足的环境中优于RGB摄像头，但其低层次特征表示导致行人检测和跟踪困难。
...
### [177] [Privacy-preserving Preselection for Face Identification Based on Packing](https://arxiv.org/abs/2507.02414)
*Rundong Xin,Taotao Wang,Jin Wang,Chonghe Zhao,Jing Wang*

Main category: cs.CV

TL;DR: 提出了一种名为PFIP的高效密文域人脸检索方案，通过预选机制和打包模块提升检索效率，实验表明其准确性和速度显著优于现有方法。
...
Result: 在LFW和CASIA数据集上，PFIP保持了原始人脸识别模型的准确性，检索1000个密文人脸模板仅需300毫秒，检索效率提升近50倍。
...
TL;DR: 论文提出了一种基于残差U-Net和集成学习的新方法，用于提高结构裂缝检测的准确性和效率，并在低分辨率图像上表现优于传统模型。


<details>
  <summary>Details</summary>
Motivation: 手动检测结构裂缝存在速度慢、一致性差和人为错误的问题，影响评估可靠性，因此需要更高效的自动化解决方案。
...
### [179] [AvatarMakeup: Realistic Makeup Transfer for 3D Animatable Head Avatars](https://arxiv.org/abs/2507.02419)
*Yiming Zhong,Xiaolin Zhang,Ligang Liu,Yao Zhao,Yunchao Wei*
...
<details>
  <summary>Details</summary>
Motivation: 当前3D高斯编辑方法在实现真实化妆效果时存在不足，特别是在动态表情、身份保持和细节控制方面。

Method: 采用从粗到细的策略，先通过Coherent Duplication方法确保一致性和身份保持，再利用Refinement Module优化细节。
...
### [180] [F^2TTA: Free-Form Test-Time Adaptation on Cross-Domain Medical Image Classification via Image-Level Disentangled Prompt Tuning](https://arxiv.org/abs/2507.02437)
*Wei Li,Jingyang Zhang,Lihao Liu,Guoan Wang,Junjun He,Yang Chen,Lixu Gu*
...
<details>
  <summary>Details</summary>
Motivation: 由于医疗数据标注成本高，且数据通常以不规则的域碎片形式到达，现有TTA方法无法有效处理这种自由形式的适应问题。

Method: 提出I-DiPT框架，结合图像不变提示和图像特定提示，并引入不确定性导向掩码（UoM）和平行图蒸馏（PGD）方法。

Result: 在乳腺癌和青光眼分类任务中，该方法优于现有TTA方法。

Conclusion: I-DiPT框架有效解决了自由形式测试时间适应问题，提升了模型在域碎片数据上的性能。
...
### [181] [Red grape detection with accelerated artificial neural networks in the FPGA's programmable logic](https://arxiv.org/abs/2507.02443)
*Sandro Costa Magalhães,Marco Almeida,Filipe Neves dos Santos,António Paulo Moreira,Jorge Dias*
...
### [182] [IGDNet: Zero-Shot Robust Underexposed Image Enhancement via Illumination-Guided and Denoising](https://arxiv.org/abs/2507.02445)
*Hailong Yan,Junjian Huang,Tingwen Huang*
...
### [183] [Weakly-supervised Contrastive Learning with Quantity Prompts for Moving Infrared Small Target Detection](https://arxiv.org/abs/2507.02454)
*Weiwei Duan,Luping Ji,Shengjia Chen,Sicheng Zhu,Jianghong Huang,Mao Ye*
...
<details>
  <summary>Details</summary>
Motivation: 红外小目标检测因目标尺寸小、背景对比度低而困难，现有方法依赖大量人工标注，成本高。
...
### [184] [CrowdTrack: A Benchmark for Difficult Multiple Pedestrian Tracking in Real Scenarios](https://arxiv.org/abs/2507.02479)
*Teng Fu,Yuwen Chen,Zhuofan Chen,Mengyang Zhao,Bin Li,Xiangyang Xue*

Main category: cs.CV

TL;DR: 提出一个名为CrowdTrack的大规模多行人跟踪数据集，旨在解决现有数据集场景简单和非真实的问题，并测试了多种SOTA模型。
...
### [185] [MedFormer: Hierarchical Medical Vision Transformer with Content-Aware Dual Sparse Selection Attention](https://arxiv.org/abs/2507.02488)
*Zunhui Xia,Hongxing Li,Libin Lan*
...
Result: 在多种医学图像识别任务中，MedFormer显著提升了性能，并展现出更高的通用性和效率。
...
### [186] [Temporally-Aware Supervised Contrastive Learning for Polyp Counting in Colonoscopy](https://arxiv.org/abs/2507.02493)
*Luca Parolari,Andrea Cherubini,Lamberto Ballan,Carlo Biffi*

Main category: cs.CV

TL;DR: 提出了一种基于时间感知的监督对比损失方法，用于结肠镜息肉计数，显著减少了碎片化率。


<details>
  <summary>Details</summary>
Motivation: 提高结肠镜筛查的成本效益，现有方法忽视时间关系，导致计数不准确。

Method: 引入时间感知的监督对比损失和时序邻接约束，改进息肉轨迹聚类。

Result: 碎片化率比现有方法减少2.2倍，性能达到新最优。

Conclusion: 时间感知对息肉计数至关重要，新方法显著提升了准确性和鲁棒性。
...
### [189] [Automatic Labelling for Low-Light Pedestrian Detection](https://arxiv.org/abs/2507.02513)
*Dimitrios Bouzoulas,Eerik Alamikkotervo,Risto Ojala*
...
Result: 在6/9情况下，生成标签训练的模型在mAP@50和mAP@50-95指标上优于人工标注。
...
### [190] [IMASHRIMP: Automatic White Shrimp (Penaeus vannamei) Biometrical Analysis from Laboratory Images Using Computer Vision and Deep Learning](https://arxiv.org/abs/2507.02519)
*Abiam Remache González,Meriem Chagour,Timon Bijan Rüth,Raúl Trapiella Cañedo,Marina Martínez Soler,Álvaro Lorenzo Felipe,Hyun-Suk Shin,María-Jesús Zamorano Serrano,Ricardo Torres,Juan-Antonio Castillo Parra,Eduardo Reyes Abad,Miguel-Ángel Ferrer Ballester,Juan-Manuel Afonso López,Francisco-Mario Hernández Tejera,Adrian Penate-Sanchez*

Main category: cs.CV

TL;DR: IMASHRIMP是一个基于深度学习和计算机视觉的系统，用于自动化分析白虾的形态，优化水产养殖中的遗传选择任务。


<details>
  <summary>Details</summary>
Motivation: 传统方法在虾类形态分析中存在高错误率，IMASHRIMP旨在减少人为错误并提高效率。

Method: 系统结合了改进的ResNet-50架构、姿态估计模块（VitPose）和SVM模型，用于图像分类、关键点检测和单位转换。

Result: 系统显著降低了错误率（视角分类0%，额角检测3.64%），姿态估计mAP达97.94%，单位转换误差为0.07cm。

Conclusion: IMASHRIMP展示了自动化虾类形态分析的潜力，有助于提升水产养殖的可持续性和效率。
...
### [191] [Addressing Camera Sensors Faults in Vision-Based Navigation: Simulation and Dataset Development](https://arxiv.org/abs/2507.02602)
*Riccardo Gallon,Fabian Schiemenz,Alessandra Menicucci,Eberhard Gill*
...
Method: 研究分析了相机传感器故障案例，开发了模拟框架生成故障图像数据集，用于训练AI故障检测算法。

Result: 提出了一个系统化的故障模拟框架，并生成了可用于AI训练的故障数据集。
...
### [192] [MoGe-2: Accurate Monocular Geometry with Metric Scale and Sharp Details](https://arxiv.org/abs/2507.02546)
*Ruicheng Wang,Sicheng Xu,Yue Dong,Yu Deng,Jianfeng Xiang,Zelong Lv,Guangzhong Sun,Xin Tong,Jiaolong Yang*
...
Method: MoGe-2在MoGe的基础上扩展，结合度量几何预测策略，并开发了统一的数据细化方法，利用合成标签过滤和补全真实数据。
...
### [193] [APT: Adaptive Personalized Training for Diffusion Models with Limited Data](https://arxiv.org/abs/2507.02687)
*JungWoo Chae,Jiyoon Kim,JaeWoong Choi,Kyungyul Kim,Sangheum Hwang*
...
Method: APT包括自适应训练调整、特征稳定化和注意力对齐三个组件。
...
### [194] [Reconstructing Close Human Interaction with Appearance and Proxemics Reasoning](https://arxiv.org/abs/2507.02565)
*Buzhen Huang,Chen Li,Chongyang Xu,Dongyue Lu,Jinnan Chen,Yangang Wang,Gim Hee Lee*
...
### [195] [FairHuman: Boosting Hand and Face Quality in Human Image Generation with Minimum Potential Delay Fairness in Diffusion Models](https://arxiv.org/abs/2507.02714)
*Yuxuan Wang,Tianwei Cao,Huayu Zhang,Zhongjiang He,Kongming Liang,Zhanyu Ma*
...
### [196] [Parametric shape models for vessels learned from segmentations via differentiable voxelization](https://arxiv.org/abs/2507.02576)
*Alina F. Dima,Suprosanna Shit,Huaqi Qiu,Robbie Holland,Tamara T. Mueller,Fabio Antonio Musio,Kaiyuan Yang,Bjoern Menze,Rickmer Braren,Marcus Makowski,Daniel Rueckert*
...
<details>
  <summary>Details</summary>
Motivation: 血管在医学研究中需要多种表示方法，但现有方法通常独立使用这些表示，缺乏统一框架。

Method: 利用可微分体素化技术，通过形状到分割的拟合自动提取血管参数化模型，使用三次B样条参数化中心线和半径，并从中提取网格。

Result: 实验证明，该方法能准确建模复杂血管（如主动脉、动脉瘤和脑血管）的几何形状。

Conclusion: 该框架实现了血管多表示的统一，为医学图像分析提供了高效工具。
...
### [197] [Linear Attention with Global Context: A Multipole Attention Mechanism for Vision and Physics](https://arxiv.org/abs/2507.02748)
*Alex Colagrande,Paul Caillon,Eva Feillet,Alexandre Allauzen*

Main category: cs.CV

TL;DR: 提出了一种名为MANO的多极注意力神经算子，通过距离多尺度方式计算注意力，解决了Transformer在高分辨率输入下的计算和内存问题。
...
### [198] [Structure-aware Semantic Discrepancy and Consistency for 3D Medical Image Self-supervised Learning](https://arxiv.org/abs/2507.02581)
*Tan Pan,Zhaorui Tan,Kaiyu Guo,Dongli Xu,Weidi Xu,Chen Jiang,Xin Guo,Yuan Qi,Yuan Cheng*

Main category: cs.CV

TL;DR: 论文提出了一种名为$S^2DC$的自监督学习框架，通过结构感知的语义差异和一致性改进3D医学图像分析。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用固定大小的图像块划分，忽略了医学图像中解剖结构的位置、尺度和形态变化，导致无法有效捕捉有意义的差异。

Method: $S^2DC$通过两步实现结构感知表示：1) 利用最优传输策略增强不同图像块的语义差异；2) 基于邻域相似性分布提升结构级别的语义一致性。

Result: 在10个数据集、4个任务和3种模态上的实验表明，$S^2DC$优于现有自监督学习方法。

Conclusion: $S^2DC$通过结构感知的语义差异和一致性，显著提升了3D医学图像自监督学习的性能。
...
### [199] [AuroraLong: Bringing RNNs Back to Efficient Open-Ended Video Understanding](https://arxiv.org/abs/2507.02591)
*Weili Xu,Enxin Song,Wenhao Chai,Xuexiang Wen,Tian Ye,Gaoang Wang*
...
<details>
  <summary>Details</summary>
Motivation: 长视频理解因计算复杂性和内存成本高而具有挑战性，传统Transformer模型的计算和内存需求随序列长度呈二次增长。

Method: 用线性RNN语言模型替代MLLM中的LLM组件，结合视觉令牌合并和排序，以恒定大小隐藏状态处理任意长度输入。

Result: AuroraLong在多个视频基准测试中表现接近类似规模的Transformer模型，仅需2B参数和公开数据训练。

Conclusion: 线性RNN模型有望降低长视频理解的计算门槛，首次在LLaVA类模型中用于开放视频理解。
...
TL;DR: 该论文提出了一种基于多注意力交互机制的综合优化方法（USAD），用于解决人类活动识别（HAR）中的标记数据稀缺、特征提取不足和设备性能问题。通过无监督数据增强、多分支时空交互网络和自适应多损失函数融合，显著提升了模型性能。
...
### [201] [AIGI-Holmes: Towards Explainable and Generalizable AI-Generated Image Detection via Multimodal Large Language Models](https://arxiv.org/abs/2507.02664)
*Ziyin Zhou,Yunpeng Luo,Yuanchen Wu,Ke Sun,Jiayi Ji,Ke Yan,Shouhong Ding,Xiaoshuai Sun,Yunsheng Wu,Rongrong Ji*
...
### [202] [Learning few-step posterior samplers by unfolding and distillation of diffusion models](https://arxiv.org/abs/2507.02686)
*Charlesquin Kemajou Mbakam,Jonathan Spence,Marcelo Pereyra*

Main category: cs.CV

TL;DR: 提出了一种结合深度展开和模型蒸馏的新框架，将扩散模型转化为高效的后验采样器，显著提升计算效率和灵活性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在贝叶斯计算成像中表现出色，但现有方法在灵活性和准确性之间存在权衡。

Method: 通过深度展开和模型蒸馏，将扩散模型转化为条件模型，结合LATINO Langevin采样器。

Result: 实验表明，该方法在准确性和计算效率上优于现有技术，同时保持灵活性。

Conclusion: 新框架成功平衡了灵活性和效率，为计算成像提供了高效解决方案。
...
### [203] [CanonSwap: High-Fidelity and Consistent Video Face Swapping via Canonical Space Modulation](https://arxiv.org/abs/2507.02691)
*Xiangyang Luo,Ye Zhu,Yunfei Liu,Lijian Lin,Cong Wan,Zijian Cai,Shao-Lun Huang,Yu Li*

Main category: cs.CV

TL;DR: CanonSwap是一种新的视频人脸交换框架，通过解耦面部外观和运动信息，显著提升了身份转移和动态属性保留的效果。
...
Method: CanonSwap通过将运动信息与外观信息解耦，在统一规范空间内进行身份修改，并通过Partial Identity Modulation模块和细粒度同步指标优化身份转移和动态属性保留。

Result: 实验表明，CanonSwap在视觉质量、时间一致性和身份保留方面显著优于现有方法。

Conclusion: CanonSwap通过解耦和优化设计，解决了视频人脸交换中的关键挑战，实现了更高质量和一致性的结果。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖隐式内存进行3D重建，但存在容量限制和早期帧信息丢失的问题。Point3R旨在通过显式内存更高效地整合场景信息。

Method: Point3R维护一个显式的空间指针内存，每个指针与3D场景结构直接关联，并通过3D分层位置嵌入和融合机制实现信息的高效整合。

Result: Point3R在多种任务中表现出竞争性或最先进的性能，且训练成本较低。
...
### [205] [SIU3R: Simultaneous Scene Understanding and 3D Reconstruction Beyond Feature Alignment](https://arxiv.org/abs/2507.02705)
*Qi Xu,Dongxu Wei,Lingzhe Zhao,Wenpu Li,Zhangchi Huang,Shunping Ji,Peidong Liu*
...
Method: SIU3R采用像素对齐的3D表示，将多任务统一为可学习查询，并设计轻量级模块促进任务协作。
...
### [206] [UniMC: Taming Diffusion Transformer for Unified Keypoint-Guided Multi-Class Image Generation](https://arxiv.org/abs/2507.02713)
*Qin Guo,Ailing Zeng,Dongxu Yue,Ceyuan Yang,Yang Cao,Hanzhong Guo,Fei Shen,Wei Liu,Xihui Liu,Dan Xu*

Main category: cs.CV

TL;DR: 论文提出了一种基于DiT的框架UniMC，用于统一控制多类图像生成，并发布了高质量数据集HAIG-2.9M，解决了现有关键点引导模型在非刚性物体和多重叠对象生成上的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有关键点引导模型在生成非刚性物体（如动物）和多重叠对象时存在局限性，主要由于方法本身的限制和缺乏合适的数据集。

Method: 设计了UniMC框架，将实例和关键点条件整合为紧凑标记，并提出了HAIG-2.9M数据集，包含丰富的标注和严格的人工检查。

Result: 实验证明HAIG-2.9M的高质量和UniMC的有效性，尤其在严重遮挡和多类场景中表现突出。
...
### [207] [Prompt learning with bounding box constraints for medical image segmentation](https://arxiv.org/abs/2507.02743)
*Mélanie Gaillochet,Mehrdad Noori,Sahar Dastani,Christian Desrosiers,Hervé Lombaert*

Main category: cs.CV

TL;DR: 提出一种结合基础模型和弱监督分割的新框架，仅使用边界框注释自动生成提示，优化方案整合了伪标签和约束条件，实验表现优于现有方法。
...
Conclusion: 新框架有效减少标注负担，提升分割性能，代码开源。
...
### [208] [DexVLG: Dexterous Vision-Language-Grasp Model at Scale](https://arxiv.org/abs/2507.02747)
*Jiawei He,Danshi Li,Xinqiang Yu,Zekun Qi,Wenyao Zhang,Jiayi Chen,Zhaoxiang Zhang,Zhizheng Zhang,Li Yi,He Wang*

Main category: cs.CV

TL;DR: DexVLG是一个大型视觉-语言-抓取模型，用于预测与语言指令对齐的灵巧抓取姿势，通过单视图RGBD输入实现。
...
Conclusion: DexVLG在灵巧抓取任务中表现出色，为未来研究提供了重要工具。
...
### [209] [Partial Weakly-Supervised Oriented Object Detection](https://arxiv.org/abs/2507.02751)
*Mingxin Liu,Peiyuan Zhang,Yuan Liu,Wei Zhang,Yue Zhou,Ning Liao,Ziyang Gong,Junwei Luo,Zhirui Wang,Yi Yu,Xue Yang*
...
### [210] [From Pixels to Damage Severity: Estimating Earthquake Impacts Using Semantic Segmentation of Social Media Images](https://arxiv.org/abs/2507.02781)
*Danrong Zhang,Huili Huang,N. Simrill Smith,Nimisha Roy,J. David Frost*

Main category: cs.CV

TL;DR: 该研究提出了一种基于语义分割的方法，用于更客观地评估地震后社交媒体图像中的破坏程度，通过构建数据集并微调SegFormer模型，实现了对破坏的量化分析。


<details>
  <summary>Details</summary>
Motivation: 传统分类方法在评估地震后社交媒体图像中的破坏程度时存在主观性，无法反映图像内不同区域的破坏差异。

Method: 研究将破坏程度评估问题转化为语义分割任务，构建了包含三种破坏程度的数据集，并微调SegFormer模型进行分割。同时，提出了一种新的破坏评分系统。

Result: 该方法能够更客观、全面地量化社交媒体图像中的破坏程度，为灾害侦察团队提供更精确的指导。

Conclusion: 通过语义分割和新的评分系统，该研究提升了地震后破坏评估的客观性和实用性，有助于更有效的灾害响应。
...
### [211] [RichControl: Structure- and Appearance-Rich Training-Free Spatial Control for Text-to-Image Generation](https://arxiv.org/abs/2507.02792)
*Liheng Zhang,Lexi Pang,Hang Ye,Xiaoxuan Ma,Yizhou Wang*

Main category: cs.CV

TL;DR: 提出了一种灵活的文本到图像扩散模型特征注入框架，解决了现有方法中的结构错位和视觉伪影问题，并通过实验验证了其优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有特征注入方法在条件图像与自然RGB分布差异较大时，存在结构错位、条件泄漏和视觉伪影等问题。

Method: 提出了一种解耦注入时间步和去噪过程的特征注入框架，结合结构丰富的注入模块、外观丰富的提示和重启细化策略。

Result: 实验表明，该方法在多样化的零样本条件场景中实现了最先进的性能。

Conclusion: 通过灵活的特征注入和优化策略，实现了无需训练的结构和外观丰富的图像生成。
...
### [212] [No time to train! Training-Free Reference-Based Instance Segmentation](https://arxiv.org/abs/2507.02798)
*Miguel Espinosa,Chenhongyi Yang,Linus Ericsson,Steven McDonagh,Elliot J. Crowley*

Main category: cs.CV

TL;DR: 论文提出了一种基于参考图像的物体分割方法，利用基础模型的语义先验自动生成分割掩码，显著提升了分割性能。
...
Conclusion: 通过语义先验和参考图像，实现了高效且自动化的物体分割。
...
### [213] [LangScene-X: Reconstruct Generalizable 3D Language-Embedded Scenes with TriMap Video Diffusion](https://arxiv.org/abs/2507.02813)
*Fangfu Liu,Hao Li,Jiawei Chi,Hanyang Wang,Minghui Yang,Fudong Wang,Yueqi Duan*
...
Method: 结合TriMap视频扩散模型生成RGB、几何和语义信息，并引入语言量化压缩器（LQC）编码语言嵌入，最后通过语言表面场对齐实现开放查询。

Result: 在真实数据上的实验显示，LangScene-X在质量和泛化性上优于现有方法。
...
### [214] [Confidence-driven Gradient Modulation for Multimodal Human Activity Recognition: A Dynamic Contrastive Dual-Path Learning Approach](https://arxiv.org/abs/2507.02826)
*Panpan Ji,Junni Song,Hang Xiao,Hanyu Liu,Chao Li*

Main category: cs.CV

TL;DR: 提出了一个名为DCDP-HAR的动态对比双路径网络框架，解决多模态HAR中的特征对齐和模态贡献不平衡问题。
...
### [215] [AnyI2V: Animating Any Conditional Image with Motion Control](https://arxiv.org/abs/2507.02857)
*Ziye Li,Hao Luo,Xincheng Shuai,Henghui Ding*

Main category: cs.CV

TL;DR: AnyI2V是一个无需训练的框架，通过用户定义的运动轨迹为条件图像生成动画，支持多种模态输入，并实现风格迁移和编辑。
...
Method: 提出AnyI2V框架，支持多种模态的条件图像输入（如网格和点云），并允许混合输入和风格迁移。

Result: 实验表明AnyI2V在空间和运动控制的视频生成中表现优异。

Conclusion: AnyI2V为视频生成提供了更灵活和多功能的方法，支持多种输入和编辑功能。
...
### [216] [Bootstrapping Grounded Chain-of-Thought in Multimodal LLMs for Data-Efficient Model Adaptation](https://arxiv.org/abs/2507.02859)
*Jiaer Xia,Bingkui Tong,Yuhang Zang,Rui Shao,Kaiyang Zhou*

Main category: cs.CV

TL;DR: MLLMs在通用图像理解上表现优异，但在专业视觉任务（如图表理解）上需要大规模数据重训练。研究发现，使用CoT推理数据可提升模型在数据有限时的适应性，但CoT数据存在事实错误。提出GCoT方法，通过注入边界框信息提升推理准确性，实验证明其在五种专业视觉任务中显著优于微调和蒸馏。


<details>
  <summary>Details</summary>
Motivation: 解决MLLMs在专业视觉任务中因预训练与下游数据集不匹配而难以适应的问题，尤其是在数据有限的情况下。

Method: 提出GCoT方法，通过将边界框信息注入CoT数据，提升推理步骤的准确性。

Result: 在五种专业视觉任务（图表、表格、收据、报告等）中，GCoT在数据有限时显著优于微调和蒸馏。

Conclusion: GCoT通过增强推理数据的忠实性，有效提升了MLLMs在专业视觉任务中的表现，尤其在数据有限时。
...
### [217] [Less is Enough: Training-Free Video Diffusion Acceleration via Runtime-Adaptive Caching](https://arxiv.org/abs/2507.02860)
*Xin Zhou,Dingkang Liang,Kaijin Chen,Tianrui Feng,Xiwu Chen,Hongkai Lin,Yikang Ding,Feiyang Tan,Hengshuang Zhao,Xiang Bai*
...
### [218] [RefTok: Reference-Based Tokenization for Video Generation](https://arxiv.org/abs/2507.02862)
*Xiang Fan,Xiaohang Sun,Kushan Thakkar,Zhu Liu,Vimal Bhat,Ranjay Krishna,Xiang Hao*

Main category: cs.CV

TL;DR: RefTok是一种基于参考的令牌化方法，能有效捕捉视频中的时间动态和上下文信息，显著优于现有方法。
...
Method: 提出RefTok方法，基于未量化的参考帧编码和解码帧集，保留运动连续性和对象外观。

Result: 在四个视频数据集上，RefTok显著优于现有方法，平均提升36.7%的指标；在视频生成任务中，性能超越更大模型。
...
### [219] [Gbake: Baking 3D Gaussian Splats into Reflection Probes](https://arxiv.org/abs/2507.02257)
...
### [220] [Real-time Image-based Lighting of Glints](https://arxiv.org/abs/2507.02674)
...
### [221] [Gbake: Baking 3D Gaussian Splats into Reflection Probes](https://arxiv.org/abs/2507.02257)
*Stephen Pasch,Joel K. Salzman,Changxi Zheng*

Main category: cs.GR

TL;DR: GBake工具用于在3D高斯泼溅场景中烘焙反射探针，以实现传统3D网格在Unity中的真实反射映射。
...
Result: 实现了传统3D网格在Unity中的真实反射映射。
...
### [222] [Real-time Image-based Lighting of Glints](https://arxiv.org/abs/2507.02674)
*Tom Kneiphof,Reinhard Klein*

Main category: cs.GR

TL;DR: 提出了一种高效近似方法，用于基于图像的闪光照明，支持动态材质和环境贴图，实时渲染效果接近真实。


<details>
  <summary>Details</summary>
Motivation: 闪光材质在实时渲染中难以处理，需要动态材质和环境贴图的支持。

Method: 采用实时闪光渲染和标准环境贴图滤波技术，通过分区滤波和概率采样实现高效渲染。
...
### [223] [STELLA: Self-Evolving LLM Agent for Biomedical Research](https://arxiv.org/abs/2507.02004)
...
### [224] [HCVR: A Hybrid Approach with Correlation-aware Voting Rules for Feature Selection](https://arxiv.org/abs/2507.02073)
...
### [225] [Reasoning on a Budget: A Survey of Adaptive and Controllable Test-Time Compute in LLMs](https://arxiv.org/abs/2507.02076)
...
### [226] [Measuring Scientific Capabilities of Language Models with a Systems Biology Dry Lab](https://arxiv.org/abs/2507.02083)
...
### [227] [What Neuroscience Can Teach AI About Learning in Continuously Changing Environments](https://arxiv.org/abs/2507.02103)
...
### [228] [The Illusion of Fairness: Auditing Fairness Interventions with Audit Studies](https://arxiv.org/abs/2507.02152)
...
### [229] [Data Diversification Methods In Alignment Enhance Math Performance In LLMs](https://arxiv.org/abs/2507.02173)
...
### [230] [Do Role-Playing Agents Practice What They Preach? Belief-Behavior Consistency in LLM-Based Simulations of Human Trust](https://arxiv.org/abs/2507.02197)
...
### [231] [Dilution, Diffusion and Symbiosis in Spatial Prisoner's Dilemma with Reinforcement Learning](https://arxiv.org/abs/2507.02211)
...
### [232] [Scaling LLM Planning: NL2FLOW for Parametric Problem Generation and Rigorous Evaluation](https://arxiv.org/abs/2507.02253)
*Jungkoo Kang*
...
### [233] [Iterated belief revision: from postulates to abilities](https://arxiv.org/abs/2507.02319)
*Paolo Liberatore*
...
### [235] [An AI-native experimental laboratory for autonomous biomolecular engineering](https://arxiv.org/abs/2507.02379)
...
### [236] [The Gauss-Markov Adjunction: Categorical Semantics of Residuals in Supervised Learning](https://arxiv.org/abs/2507.02442)
*Moto Kamiura*
...
### [237] [Clarifying Before Reasoning: A Coq Prover with Structural Context](https://arxiv.org/abs/2507.02541)
...
### [238] [AI Research Agents for Machine Learning: Search, Exploration, and Generalization in MLE-bench](https://arxiv.org/abs/2507.02554)
...
### [239] [Responsibility Gap and Diffusion in Sequential Decision-Making Mechanisms](https://arxiv.org/abs/2507.02582)
...
### [240] [DynamiCare: A Dynamic Multi-Agent Framework for Interactive and Open-Ended Medical Decision-Making](https://arxiv.org/abs/2507.02616)
...
### [241] [Strategic Intelligence in Large Language Models: Evidence from evolutionary Game Theory](https://arxiv.org/abs/2507.02618)
...
### [242] [Decoupled Planning and Execution: A Hierarchical Reasoning Framework for Deep Search](https://arxiv.org/abs/2507.02652)
...
### [243] [Hey AI, Generate Me a Hardware Code! Agentic AI-based Hardware Design & Verification](https://arxiv.org/abs/2507.02660)
...
### [244] [Think How to Think: Mitigating Overthinking with Autonomous Difficulty Cognition in Large Reasoning Models](https://arxiv.org/abs/2507.02663)
...
### [245] [Detection of Disengagement from Voluntary Quizzes: An Explainable Machine Learning Approach in Higher Distance Education](https://arxiv.org/abs/2507.02681)
...
### [246] [Time-critical and confidence-based abstraction dropping methods](https://arxiv.org/abs/2507.02703)
...
### [247] [Bourbaki: Self-Generated and Goal-Conditioned MDPs for Theorem Proving](https://arxiv.org/abs/2507.02726)
...
### [248] [Knowledge Protocol Engineering: A New Paradigm for AI in Domain-Specific Knowledge Work](https://arxiv.org/abs/2507.02760)
*Guangwei Zhang*
...
### [249] [Grounding Intelligence in Movement](https://arxiv.org/abs/2507.02771)
...
### [250] [KERAP: A Knowledge-Enhanced Reasoning Approach for Accurate Zero-shot Diagnosis Prediction Using Multi-agent LLMs](https://arxiv.org/abs/2507.02773)
...
### [251] [Moral Responsibility or Obedience: What Do We Want from AI?](https://arxiv.org/abs/2507.02788)
*Joseph Boland*
...
### [252] [Establishing Best Practices for Building Rigorous Agentic Benchmarks](https://arxiv.org/abs/2507.02825)
...
### [253] [StepHint: Multi-level Stepwise Hints Enhance Reinforcement Learning to Reason](https://arxiv.org/abs/2507.02841)
...
### [254] [STELLA: Self-Evolving LLM Agent for Biomedical Research](https://arxiv.org/abs/2507.02004)
*Ruofan Jin,Zaixi Zhang,Mengdi Wang,Le Cong*
...
Method: 采用多代理架构，包含动态工具库和模板库，通过工具创建代理自动发现和整合新工具。

Result: 在多个生物医学基准测试中表现优异，性能随经验显著提升。

Conclusion: STELLA代表了AI代理系统的重要进步，能够动态扩展专业知识以加速生物医学发现。
...
### [255] [HCVR: A Hybrid Approach with Correlation-aware Voting Rules for Feature Selection](https://arxiv.org/abs/2507.02073)
*Nikita Bhedasgaonkar,Rushikesh K. Joshi*
...
<details>
  <summary>Details</summary>
Motivation: 传统特征选择方法（如CFS、mRMR、MI等）在性能和效率上存在不足，需要一种更高效的方法。

Method: HCVR结合非迭代和迭代过滤方法，通过后向消除和多数投票规则选择特征。

Result: 在SPAMBASE数据集上，HCVR表现优于传统非迭代和迭代方法。
...
### [256] [Reasoning on a Budget: A Survey of Adaptive and Controllable Test-Time Compute in LLMs](https://arxiv.org/abs/2507.02076)
*Mohammad Ali Alomrani,Yingxue Zhang,Derek Li,Qianyi Sun,Soumyasundar Pal,Zhanguang Zhang,Yaochen Hu,Rohan Deepak Ajwani,Antonios Valkanas,Raika Karimi,Peng Cheng,Yunzhou Wang,Pengyi Liao,Hanrui Huang,Bin Wang,Jianye Hao,Mark Coates*

Main category: cs.AI

TL;DR: 本文综述了高效测试时计算（TTC）策略，旨在提升大语言模型（LLMs）的计算效率，提出了两级分类法，并比较了不同方法的性能与计算成本。
...
Method: 提出两级分类法：L1-可控性（固定计算预算）和L2-自适应性（动态调整计算资源）。对主流LLMs进行基准测试。

Result: 揭示了推理性能与计算成本之间的权衡，强调了TTC方法的实际控制性、适应性和可扩展性。

Conclusion: 未来研究方向包括混合思维模型，目标是提升LLMs的计算效率、鲁棒性和用户响应能力。
...
### [257] [Measuring Scientific Capabilities of Language Models with a Systems Biology Dry Lab](https://arxiv.org/abs/2507.02083)
*Haonan Duan,Stephen Zhewen Lu,Caitlin Fiona Harrigan,Nishkrit Desai,Jiarui Lu,Michał Koziarski,Leonardo Cotta,Chris J. Maddison*
...
Result: 性能更强的模型表现更优，但所有模型在系统复杂性增加时性能显著下降，表明LLM的科学能力仍有很大提升空间。
...
### [258] [What Neuroscience Can Teach AI About Learning in Continuously Changing Environments](https://arxiv.org/abs/2507.02103)
*Daniel Durstewitz,Bruno Averbeck,Georgia Koppe*
...
Result: 提出神经科学如何指导AI发展的具体议程，促进NeuroAI领域的双向交流。
...
### [259] [The Illusion of Fairness: Auditing Fairness Interventions with Audit Studies](https://arxiv.org/abs/2507.02152)
*Disa Sariola,Patrick Button,Aron Culotta,Nicholas Mattei*
...
<details>
  <summary>Details</summary>
Motivation: 研究AI系统在复杂决策（如招聘）中的公平性和有效性，传统数据采样方法存在选择偏差和标签偏差，审计研究数据能提供更高质量的数据支持。

Method: 使用审计研究数据（如虚构简历的随机对照试验）来训练和评估算法，并引入基于个体治疗效果估计的新干预方法。

Result: 发现传统公平干预方法（如均衡基础率）在传统指标下看似公平，但实际存在约10%的偏差；新干预方法进一步减少了算法歧视。

Conclusion: 审计研究数据能更准确地揭示和减少算法歧视，为公平性干预提供了更可靠的基础。
...
### [260] [Data Diversification Methods In Alignment Enhance Math Performance In LLMs](https://arxiv.org/abs/2507.02173)
*Berkan Dokmeci,Qingyang Wu,Ben Athiwaratkun,Ce Zhang,Shuaiwen Leon Song,James Zou*

Main category: cs.AI

TL;DR: 通过数据多样化策略（如Diversified-Think-Solve）提升大语言模型的数学推理能力，优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 数学推理是大语言模型持续面临的挑战，研究如何通过偏好学习的数据多样化策略提升其能力。

Method: 评估三种数据生成方法（温度采样、思维链提示、蒙特卡洛树搜索），并提出Diversified-Think-Solve（DTS）方法，系统分解问题为多样化推理路径。

Result: DTS显著提升数学推理性能（GSM8K提升7.1%，MATH提升4.2%），计算开销仅增加1.03倍，优于蒙特卡洛树搜索。
...
### [261] [Do Role-Playing Agents Practice What They Preach? Belief-Behavior Consistency in LLM-Based Simulations of Human Trust](https://arxiv.org/abs/2507.02197)
*Amogh Mannekote,Adam Davies,Guohao Li,Kristy Elizabeth Boyer,ChengXiang Zhai,Bonnie J Dorr,Francesco Pinto*
...
### [262] [Dilution, Diffusion and Symbiosis in Spatial Prisoner's Dilemma with Reinforcement Learning](https://arxiv.org/abs/2507.02211)
*Gustavo C. Mangold,Heitor C. M. Fernandes,Mendeli H. Vainstein*
...
Result: 观察到多种效应，包括固定更新规则与学习规则在游戏中的定性等价性，以及多动作定义下种群间的共生互利效应。
...
### [263] [Scaling LLM Planning: NL2FLOW for Parametric Problem Generation and Rigorous Evaluation](https://arxiv.org/abs/2507.02253)
*Jungkoo Kang*

Main category: cs.AI

TL;DR: NL2FLOW是一个自动化系统，用于生成自然语言规划问题并评估LLM生成的计划质量，结果显示高性能模型在生成有效计划和最优计划方面表现良好，但中间步骤可能降低性能。
...
Result: 生成2296个问题数据集，最高性能模型在生成有效计划和最优计划上的成功率分别为86%和69%。中间翻译步骤可能降低性能。

Conclusion: 直接推理可能优于分解任务，动态理解LLM的局限性对发挥其作为智能问题解决者的潜力至关重要。
...
### [264] [Iterated belief revision: from postulates to abilities](https://arxiv.org/abs/2507.02319)
*Paolo Liberatore*
...
<details>
  <summary>Details</summary>
Motivation: 当前信念修正研究过于依赖后设条件，忽略了修正机制的实际能力，如能否达到特定信念状态。

Method: 通过分析不同修正机制（如词汇、自然、激进等）的能力，探讨其是否能实现塑性、平等化、教条化等目标。

Result: 研究发现不同修正机制具备不同的能力，如某些机制能实现教条化信念，而另一些则不能。

Conclusion: 信念修正机制的能力分析比单纯的后设条件更全面，为实际应用提供了更灵活的选择。
...
### [265] [OMS: On-the-fly, Multi-Objective, Self-Reflective Ad Keyword Generation via LLM Agent](https://arxiv.org/abs/2507.02353)
*Bowen Chen,Zhao Wang,Shingo Takamatsu*

Main category: cs.AI

TL;DR: OMS框架解决了LLM在关键词生成中的三大限制，无需训练数据、多目标优化和自反思，实验证明其优于现有方法。
...
Conclusion: OMS框架通过实时监控和多目标优化，显著提升了关键词生成的质量和效果。
...
### [266] [An AI-native experimental laboratory for autonomous biomolecular engineering](https://arxiv.org/abs/2507.02379)
*Mingyu Wu,Zhaoguo Wang,Jiabin Wang,Zhiyuan Dong,Jingkai Yang,Qingting Li,Tianyu Huang,Lei Zhao,Mingqiang Li,Fei Wang,Chunhai Fan,Haibo Chen*

Main category: cs.AI

TL;DR: AI驱动的自主实验室，支持复杂多目标实验，实现高效仪器利用和实验优化。


<details>
  <summary>Details</summary>
Motivation: 实现非专家也能进行复杂科学实验的自主研究，突破传统实验的限制。

Method: 采用AI原生设计，结合模型、实验和仪器的协同优化，支持多用户请求和复杂实验流程。

Result: 自主实验室在核酸功能实验中达到与人类科学家相当的水平，显著提升仪器利用率和实验效率。

Conclusion: 该平台为生物材料研究提供了无专家依赖的解决方案，推动了科学即服务的大规模实现。
...
### [267] [The Gauss-Markov Adjunction: Categorical Semantics of Residuals in Supervised Learning](https://arxiv.org/abs/2507.02442)
*Moto Kamiura*

Main category: cs.AI

TL;DR: 论文通过范畴论重新构建机器学习模型，提出了一种语义框架来增强AI系统的可理解性和可解释性，重点研究了多元线性回归模型，并引入了Gauss-Markov Adjunction来描述参数与残差之间的结构关系。


<details>
  <summary>Details</summary>
Motivation: 提升机器学习的可理解性和可解释性，以满足AI可解释性的需求，并促进AI在社会中的更好应用。

Method: 利用范畴论重新构建机器学习模型，定义了两个具体范畴（参数和数据）及其间的伴随函子，提出了Gauss-Markov Adjunction来描述信息流。

Result: 通过右伴随函子保持极限的性质，证明了普通最小二乘估计与最小残差之间的关系，并将此框架定位为监督学习的扩展指称语义实例。

Conclusion: 该研究为AI的可解释性提供了形式化基础，并提出了将理论计算机科学中的语义视角应用于AI可解释性的方法。
...
### [268] [Clarifying Before Reasoning: A Coq Prover with Structural Context](https://arxiv.org/abs/2507.02541)
*Yanzhen Lu,Hanbin Yang,Xiaodie Wang,Ge Zhang,Biao Li,Chenxu Fu,Chao Li,Yang Yuan,Andrew Chi-Chih Yao*
...
Method: 引入概念级指标评估任务清晰度，通过结构化语义上下文增强输入，采用Planner-Executor架构。

Result: 清晰度得分提升1.85倍（44.5%→82.3%），证明成功率提高2.1倍（21.8%→45.8%），超越之前最佳模型。
...
### [269] [AI Research Agents for Machine Learning: Search, Exploration, and Generalization in MLE-bench](https://arxiv.org/abs/2507.02554)
*Edan Toledo,Karen Hambardzumyan,Martin Josifoski,Rishi Hazra,Nicolas Baldwin,Alexis Audran-Reiss,Michael Kuchnik,Despoina Magka,Minqi Jiang,Alisia Maria Lupidi,Andrei Lupu,Roberta Raileanu,Kelvin Niu,Tatiana Shavrina,Jean-Christophe Gagnon-Audet,Michael Shvartsman,Shagun Sodhani,Alexander H. Miller,Abhishek Charnalia,Derek Dunfield,Carole-Jean Wu,Pontus Stenetorp,Nicola Cancedda,Jakob Nicolaus Foerster,Yoram Bachrach*

Main category: cs.AI

TL;DR: AI研究代理通过优化搜索策略和操作符集，在MLE-bench基准上取得了显著性能提升。
...
### [270] [Responsibility Gap and Diffusion in Sequential Decision-Making Mechanisms](https://arxiv.org/abs/2507.02582)
*Junli Jiang,Pavel Naumov*

Main category: cs.AI

TL;DR: 论文研究了集体决策中责任的两个重要属性（扩散和间隙）的计算复杂性，发现扩散自由和间隙自由机制的复杂度分别为Π₂-complete和Π₃-complete，而两者的交集仍为Π₂-complete。
...
Conclusion: 研究揭示了责任属性的计算复杂性，为集体决策机制的设计提供了理论支持。
...
### [271] [DynamiCare: A Dynamic Multi-Agent Framework for Interactive and Open-Ended Medical Decision-Making](https://arxiv.org/abs/2507.02616)
*Tianqi Shang,Weiqing He,Charles Zheng,Lingyao Li,Li Shen,Bingxin Zhao*

Main category: cs.AI

TL;DR: 论文提出了MIMIC-Patient数据集和DynamiCare框架，用于模拟动态、多轮互动的临床诊断过程，填补了现有单轮任务框架的不足。


<details>
  <summary>Details</summary>
Motivation: 现有医疗决策模拟框架多关注单轮任务，与现实诊断过程的不确定性、互动性和迭代性不符。

Method: 基于MIMIC-III EHRs构建MIMIC-Patient数据集，并提出DynamiCare动态多智能体框架，模拟多轮互动诊断。
...
### [272] [Strategic Intelligence in Large Language Models: Evidence from evolutionary Game Theory](https://arxiv.org/abs/2507.02618)
*Kenneth Payne,Baptiste Alloui-Cros*
...
<details>
  <summary>Details</summary>
Motivation: 探讨LLMs是否具备战略智能，能在竞争环境中进行目标推理。

Method: 通过进化迭代囚徒困境锦标赛，测试LLMs与经典策略的对抗，并分析其决策过程。

Result: LLMs表现出高度竞争力，不同模型展现出独特的战略特征（如Google的Gemini模型更具攻击性，OpenAI模型更合作）。

Conclusion: LLMs能够进行复杂的战略推理，连接了经典博弈论与机器心理学，为不确定性下的算法决策提供了新视角。
...
### [273] [Decoupled Planning and Execution: A Hierarchical Reasoning Framework for Deep Search](https://arxiv.org/abs/2507.02652)
*Jiajie Jin,Xiaoxi Li,Guanting Dong,Yuyao Zhang,Yutao Zhu,Yang Zhao,Hongjin Qian,Zhicheng Dou*
...
### [274] [Hey AI, Generate Me a Hardware Code! Agentic AI-based Hardware Design & Verification](https://arxiv.org/abs/2507.02660)
*Deepak Narayan Gadde,Keerthan Kopparam Radhakrishna,Vaisakh Naduvodi Viswambharan,Aman Kumar,Djones Lettnin,Wolfgang Kunz,Sebastian Simon*
...
<details>
  <summary>Details</summary>
Motivation: 现代集成电路设计复杂且验证过程繁琐，需要高效解决方案。

Method: 采用代理AI与人类协作的动态迭代方法，进行端到端硬件设计与验证。
...
### [275] [Think How to Think: Mitigating Overthinking with Autonomous Difficulty Cognition in Large Reasoning Models](https://arxiv.org/abs/2507.02663)
*Yongjiang Liu,Haoxi Li,Xiaosong Ma,Jie Zhang,Song Guo*
...
<details>
  <summary>Details</summary>
Motivation: 现有的长推理模型（LRMs）在处理复杂任务时存在过度思考的问题，论文旨在通过增强模型对任务难度和冗余的认知能力来解决这一问题。

Method: TH2T策略分为两阶段：1）通过难度催眠增强模型对任务难度的敏感性；2）通过冗余催眠引导模型识别并减少推理步骤中的冗余结构。

Result: 实验表明，TH2T在7B/14B/32B模型上显著降低了推理成本（简单任务超过70%，困难任务40%），同时保持了性能稳定。

Conclusion: TH2T策略有效解决了LRMs的过度思考问题，提升了模型的推理效率和输出简洁性。
...
### [276] [Detection of Disengagement from Voluntary Quizzes: An Explainable Machine Learning Approach in Higher Distance Education](https://arxiv.org/abs/2507.02681)
*Behnam Parsaeifard,Christof Imhof,Tansu Pancar,Ioan-Sorin Comsa,Martin Hlosta,Nicole Bergamin,Per Bergamin*

Main category: cs.AI

TL;DR: 论文通过分析远程教育中学生的非强制性测验参与数据，使用机器学习算法和SHAP方法，开发了一个可解释的框架来检测学生的不投入行为，准确率达91%，并探讨了及时干预的设计。


<details>
  <summary>Details</summary>
Motivation: 远程教育中学生的不投入行为可能导致学业失败，尤其是非强制性任务的参与度低。研究旨在通过数据分析和机器学习预测不投入行为，以便及时干预。
...
### [277] [Time-critical and confidence-based abstraction dropping methods](https://arxiv.org/abs/2507.02703)
*Robin Schmöcker,Lennart Kampmann,Alexander Dockhorn*
...
Method: 提出OGA-IAAD（适用于时间关键场景）和OGA-CAD（提升固定迭代次数下的性能）两种方案。
...
### [278] [Bourbaki: Self-Generated and Goal-Conditioned MDPs for Theorem Proving](https://arxiv.org/abs/2507.02726)
*Matthieu Zimmer,Xiaotong Ji,Rasul Tutunov,Anthony Bordg,Jun Wang,Haitham Bou Ammar*

Main category: cs.AI

TL;DR: 论文提出了一种自生成目标条件MDP框架（sG-MDPs），用于解决大型语言模型在自动定理证明中面临的复杂推理挑战，并在PutnamBench上取得了新突破。
...
Method: 引入自生成目标条件MDPs（sG-MDPs），通过代理生成并追求子目标，结合蒙特卡洛树搜索（MCTS）算法，构建了Bourbaki（7B）系统。
...
### [279] [Knowledge Protocol Engineering: A New Paradigm for AI in Domain-Specific Knowledge Work](https://arxiv.org/abs/2507.02760)
*Guangwei Zhang*
...
### [280] [Grounding Intelligence in Movement](https://arxiv.org/abs/2507.02771)
*Melanie Segado,Felipe Parodi,Jordan K. Matelsky,Michael L. Platt,Eva B. Dyer,Konrad P. Kording*

Main category: cs.AI

TL;DR: 论文主张将运动作为AI建模的核心目标，强调其跨领域、结构化及物理基础特性，以推动生成建模和控制能力。


<details>
  <summary>Details</summary>
Motivation: 运动在生物和人工系统中具有核心意义，但现有AI模型常忽视其结构化特性，导致数据收集和建模的碎片化。

Method: 提出将运动视为独立且丰富的模态，利用其低维表示（如姿态）进行建模，以提升可解释性和计算效率。

Result: 通过跨领域运动数据的建模，不仅能提升生成和控制能力，还能为理解智能系统行为提供统一框架。

Conclusion: 运动是智能系统与世界互动的窗口，应作为AI建模的主要目标，以促进跨领域理解和应用。
...
### [281] [KERAP: A Knowledge-Enhanced Reasoning Approach for Accurate Zero-shot Diagnosis Prediction Using Multi-agent LLMs](https://arxiv.org/abs/2507.02773)
*Yuzhang Xie,Hejie Cui,Ziyang Zhang,Jiaying Lu,Kai Shu,Fadi Nahab,Xiao Hu,Carl Yang*

Main category: cs.AI

TL;DR: KERAP是一种基于知识图谱的多智能体架构，通过增强大型语言模型（LLM）的诊断预测能力，解决了其幻觉和缺乏结构化推理的问题。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习模型依赖监督训练，泛化能力有限；而LLM在诊断预测中存在幻觉和缺乏结构化推理的问题。

Method: 提出KERAP框架，包含属性映射的链接智能体、结构化知识提取的检索智能体，以及迭代优化诊断预测的预测智能体。
...
### [282] [Moral Responsibility or Obedience: What Do We Want from AI?](https://arxiv.org/abs/2507.02788)
*Joseph Boland*

Main category: cs.AI

TL;DR: 论文探讨AI安全实践需从单纯服从转向评估伦理判断，以避免误判AI行为并维护公众信任。
...
Method: 通过分析LLMs的安全测试案例，结合哲学讨论（工具理性、道德责任等），对比不同风险范式。
...
### [283] [Establishing Best Practices for Building Rigorous Agentic Benchmarks](https://arxiv.org/abs/2507.02825)
*Yuxuan Zhu,Tengjun Jin,Yada Pruksachatkun,Andy Zhang,Shu Liu,Sasha Cui,Sayash Kapoor,Shayne Longpre,Kevin Meng,Rebecca Weiss,Fazl Barez,Rahul Gupta,Jwala Dhamala,Jacob Merizian,Mario Giulianelli,Harry Coppock,Cozmin Ududec,Jasjeet Sekhon,Jacob Steinhardt,Antony Kellerman,Sarah Schwettmann,Matei Zaharia,Ion Stoica,Percy Liang,Daniel Kang*
...
Method: 通过分析现有基准测试的问题，结合经验和最佳实践，提出ABC指南。
...
### [284] [StepHint: Multi-level Stepwise Hints Enhance Reinforcement Learning to Reason](https://arxiv.org/abs/2507.02841)
*Kaiyi Zhang,Ang Lv,Jinpeng Li,Yongbo Wang,Feng Wang,Haoyuan Hu,Rui Yan*

Main category: cs.AI

TL;DR: StepHint是一种新的RLVR算法，通过多级逐步提示解决近失误奖励问题和探索停滞问题，提高训练效率和模型推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前RLVR方法面临近失误奖励问题和探索停滞问题，限制了训练效率和模型推理能力的提升。
