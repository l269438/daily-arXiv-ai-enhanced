...
TL;DR: 提出了一个统一的多线路地铁乘务规划与重规划优化框架，采用分层时空网络模型和列生成算法，在跨线路协调和紧急情况下显著提升运营效率和成本效益


<details>
  <summary>Details</summary>
Motivation: 随着地铁网络快速扩张，现有研究主要关注单线路规划，缺乏对跨线路协调和紧急情况下快速重规划的关注，需要解决多线路地铁乘务规划中的异构人员配置和应急管理问题

Method: 提出分层时空网络模型表示统一的乘务行动空间，推导计算高效的约束条件和公式来处理乘务人员的异构资质和偏好，开发基于列生成和最短路径调整的求解算法

Result: 在上海和北京地铁真实数据上的实验表明，该方法在成本降低和任务完成方面优于基准启发式算法，通过跨线路操作实现了显著的效率提升，特别是在紧急中断情况下
...
<details>
  <summary>Details</summary>
Motivation: LLM在渗透测试自动化中的应用日益增多，但其在不同攻击阶段的有效性和可靠性尚不明确，需要系统评估和性能改进

Method: 通过构建从单智能体到模块化设计的多种LLM智能体架构，在真实渗透测试场景中进行综合评估，并针对五种核心功能进行定向增强：全局上下文记忆、智能体间消息传递、上下文条件调用、自适应规划和实时监控

Result: 研究显示，虽然某些架构天然具备部分功能特性，但定向增强显著提升了模块化智能体在复杂、多步骤和实时渗透测试任务中的性能

Conclusion: 通过核心功能增强可以大幅提高LLM智能体在渗透测试中的表现，为自动化安全测试提供有效解决方案
...
TL;DR: 提出了一个模块化评估框架，用于分解网络代理的中间步骤进行细粒度错误分析，以发现标准指标遗漏的可操作弱点


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的网络代理评估主要关注整体成功率，而忽略了中间错误，这限制了故障模式洞察和系统性改进

Method: 提出模块化评估框架，将代理管道分解为可解释的阶段进行详细错误分析，使用SeeAct框架和Mind2Web数据集作为案例研究

Result: 该方法揭示了标准指标遗漏的可操作弱点，为开发更鲁棒和可泛化的网络代理铺平了道路
...
TL;DR: VCBench是首个用于预测风险投资创始人成功的基准测试，包含9000个匿名创始人档案，评估显示LLM模型表现超越人类基准和顶级投资机构。


<details>
  <summary>Details</summary>
Motivation: 现有基准如SWE-bench和ARC-AGI在推动AGI发展方面取得显著成效，但在风险投资这一信号稀疏、结果不确定的领域缺乏标准化评估工具，需要建立可重现且保护隐私的评估标准。

Method: 创建包含9000个匿名创始人档案的VCBench数据集，通过标准化处理保留预测特征同时防止身份泄露，使用对抗测试降低90%以上的重识别风险，并评估9个最先进的大语言模型。

Result: 市场基准精度为1.9%，Y Combinator比基准高1.7倍，顶级投资机构高2.9倍。DeepSeek-V3达到基准精度的6倍以上，GPT-4o获得最高F0.5分数，大多数模型超越人类基准。

Conclusion: VCBench作为公开演进资源，为早期风险预测领域的AGI评估建立了社区驱动的标准，展示了LLM在复杂预测任务中的卓越表现。
...
TL;DR: 提出了基于人类大脑启发的真正智能(TI)新范式，包含六个核心组件，并建立了五级AGI分类法，为AGI研究提供了明确的机制导向发展路线图
...
Result: 提出了首个基于机制的整体性AGI定义，为研究社区提供了清晰可行的路径，认为实现五级AGI在功能上等同于真正智能

Conclusion: 一旦系统通过实现所有五个可测量组件达到五级AGI，其与真正智能的区别就纯粹是哲学辩论，五级AGI在功能和实践上等同于真正智能
...
TL;DR: 本文应用贝叶斯测量布局方法分析Melting Pot竞赛中多智能体系统的能力特征，发现高亲社会能力并不总是带来更好表现，且顶级参赛方案可能在不需要合作的场景中表现更好，揭示了评估框架的局限性。
...
Result: 能力特征不仅能预测未来表现，还揭示了智能体的亲社会能力；高亲社会能力不一定带来更好表现；顶级参赛方案更可能在不需要亲社会能力的场景中获得高分；竞赛获胜者可能使用了针对特定环境的硬编码方案。

Conclusion: 测量布局方法提供了强预测准确性和可操作见解，有助于在复杂社交环境中更透明、可泛化地评估AI系统。需要改进合作需求标注并考虑不同测试环境引入的偏差。
...
<details>
  <summary>Details</summary>
Motivation: 现有NL2SQL方法在任务分解和关键词提取方面存在瓶颈，导致SQL生成错误。现有数据集存在任务过度碎片化和缺乏领域特定关键词标注的问题
...
TL;DR: 提出了第一个评估大语言模型全方位理性的基准测试，涵盖多个领域和模型，包含易用工具包和实验结果分析


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型展现类人能力，需要评估它们在何种情况下像真实人类一样思考和行动，理性是评估人类行为最重要的概念之一

Method: 开发了一个全面的基准测试框架，包括理论理性和实践理性的评估，提供工具包和实验分析

Result: 基准测试揭示了LLMs与理想化人类理性的趋同和分歧点，为开发者和用户提供了基础性评估工具

Conclusion: 该基准可作为评估LLMs理性的基础工具，帮助理解模型与人类理性行为的相似性和差异性
...
TL;DR: 提出先验动态框架用于自动化工作流构建，结合Q-table学习和实时任务评估，显著提升效率并降低计算成本


<details>
  <summary>Details</summary>
Motivation: 现有工作流构建方法过度依赖历史经验，缺乏对任务特性的灵活响应，导致效率和适应性受限

Method: 基于Q-table学习优化决策空间，结合任务进度评估进行先验决策，并采用冷启动初始化、早停和剪枝机制

Result: 在四个基准数据集上平均提升4.05%，工作流构建和推理成本降至现有方法的30.68%-48.31%

Conclusion: 该框架有效平衡历史经验和任务特性响应，显著提升工作流构建的效率和适应性
...
TL;DR: 本文提出了一个针对敏感领域文本生成的差分隐私评估框架，通过大规模实证研究揭示了现有方法在复杂领域下的性能局限，并开发了针对合成文本的成员推理攻击方法。


<details>
  <summary>Details</summary>
Motivation: 在医疗和金融等高风险领域，数据共享面临监管和隐私障碍，现有匿名化方法对非结构化文本效果有限，需要具有正式隐私保证的合成数据生成方法。

Method: 1) 建立包含9个领域特定数据集的综合评估框架；2) 对最先进的差分隐私文本生成方法和不同规模的LLM进行大规模基准测试；3) 开发针对合成文本的成员推理攻击方法。

Result: 研究发现高质量领域特定合成数据生成在差分隐私约束下仍是一个未解决的挑战，性能随领域复杂性增加而下降，且公开数据集的使用可能使声称的隐私保证失效。

Conclusion: 研究强调了严格隐私审计的紧迫性，揭示了开放领域与专业评估之间的差距，为在隐私敏感高风险环境中负责任地部署生成式AI提供了指导。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在复杂多智能体工作流中的广泛应用，组织面临当前评估方法无法捕捉的错误、涌现行为和系统性故障风险，需要专门的监控调试工具。
...
TL;DR: 应用Schoenfeld认知理论分析大语言模型的推理过程，创建首个细粒度机器推理标注基准，揭示LRM推理的认知状态转换模式。


<details>
  <summary>Details</summary>
Motivation: 缺乏理解大语言模型推理结构的理论框架，需要基于人类认知理论的系统性分析方法来解读机器推理过程。

Method: 应用Schoenfeld的Episode Theory认知框架，对模型生成的数学问题解决方案进行句子和段落级标注，使用7个认知标签（如计划、实施、验证）进行细粒度分析。

Result: 创建了首个公开可用的机器推理细粒度分析基准，包含大规模标注语料和详细标注指南，初步分析揭示了LRM推理中认知状态转换的独特模式。
...
TL;DR: RationAnomaly是一个结合思维链微调和强化学习的日志异常检测框架，通过专家驱动的数据校正和多方面奖励函数，显著提升了检测准确性和逻辑一致性。
...
Method: 首先通过思维链引导的监督微调注入专家级推理模式，使用专家校正的高质量数据集；然后通过具有多方面奖励函数的强化学习阶段优化准确性和逻辑一致性。

Result: RationAnomaly在关键基准测试中优于最先进的基线方法，实现了更高的F1分数，并提供透明的逐步分析输出。

Conclusion: 该框架通过结合思维链微调和强化学习，有效解决了日志异常检测中的幻觉问题，提供了更可靠和可解释的检测方案。
...
TL;DR: 提出了Nazonazo基准测试，使用日本儿童谜语来测试基于洞察力的推理能力，解决了LLM评估中的基准饱和和污染问题。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型评估中的基准饱和和污染问题，需要成本效益高、可扩展且易于更新的评估方法。

Method: 使用日本儿童谜语构建Nazonazo基准测试，包含120个谜语项目，评估38个前沿模型和126名成年人，项目短小且无需专业知识。

Result: 除GPT-5外，没有模型能达到人类52.9%的平均准确率。推理模型显著优于非推理模型，模型大小与准确率无可靠关联。发现模型存在验证失败问题。

Conclusion: Nazonazo提供了一种成本效益高、可扩展且易于更新的基准测试格式，解决了当前评估危机，并揭示了元认知弱点。
...
TL;DR: 提出了对抗性协作RAG框架(AC-RAG)，通过两个异构代理的对抗性协作来解决检索幻觉问题，显著提升检索准确性和性能


<details>
  <summary>Details</summary>
Motivation: 当前检索增强生成(RAG)方法存在检索幻觉问题，即微调模型无法识别和处理低质量检索文档，导致性能下降

Method: 采用对抗性协作框架，包含通用检测器和领域专业解析器两个异构代理，在调解器指导下进行对抗性协作，通过持续提问和迭代问题分解来优化知识检索
...
Method: 开发OpenLens AI框架，集成专业代理进行文献综述、数据分析、代码生成和论文撰写，采用视觉语言反馈处理医学可视化，并建立质量控制机制确保可重复性，实现从研究到发表的全流程自动化。

Result: 该框架能够自动化整个研究流程，生成可直接发表的LaTeX论文，提供透明可追溯的工作流程，为健康信息学研究提供领域适配的解决方案。
...
TL;DR: 开发可解释AI框架预测碳青霉烯酶肠杆菌感染风险，使用Transformer模型分析电子病历数据，发现感染相关特征和网络变量是关键风险因素


<details>
  <summary>Details</summary>
Motivation: 碳青霉霉烯酶肠杆菌(CPE)在医院感染防控中至关重要，但使用现代深度学习方法预测CPE相关风险(如再入院、死亡率和住院时间延长)的研究仍不足

Method: 使用基于Transformer的架构和传统机器学习模型，分析爱尔兰医院的电子病历数据，包括诊断代码、病房转移、人口统计学、感染相关变量和接触网络特征，并应用XAI技术解释模型决策

Result: TabTransformer模型在多个临床预测任务中表现最佳，特别是在CPE获取预测方面(AUROC和敏感性)。发现感染相关特征(历史医院暴露、入院背景、网络中心性度量)对预测患者结局和CPE获取风险具有高度影响力

Conclusion: 研究提出了一个稳健且可解释的AI框架，用于分析复杂EMR数据以识别关键风险因素和预测CPE相关结局。Transformer模型表现优异，强调了多样化临床和网络特征的重要性
...
TL;DR: 提出了一种新型多智能体系统安全架构，包含哨兵智能体和协调器智能体的双层安全机制，通过LLM语义分析、行为分析和异常检测等技术增强MAS的安全性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统面临日益复杂的安全威胁，如提示注入、幻觉生成、隐私泄露和协同攻击等，需要动态自适应的安全防护机制来保障系统完整性。

Method: 采用分布式哨兵智能体网络进行持续监控，结合协调器智能体进行策略管理和威胁响应，集成语义分析、行为分析、检索增强验证和跨智能体异常检测等技术。

Result: 在模拟环境中成功检测了162次合成攻击（提示注入、幻觉和数据窃取），验证了监控方法的可行性，提供了增强的系统可观测性和策略演进能力。

Conclusion: 该双层安全架构为多智能体系统提供了有效的动态防御机制，能够应对多种安全威胁，同时支持监管合规和策略持续优化。
...
TL;DR: 本文提出了量化双极论证图中一组论证对目标论证最终强度贡献的函数，是现有单一论证贡献函数的泛化，并进行了基于原则的分析。


<details>
  <summary>Details</summary>
Motivation: 现有的论证贡献函数主要关注单个论证的贡献，缺乏对论证集合整体贡献的量化方法，特别是在双极论证图中需要考虑论证间的相互作用。

Method: 提出集合贡献函数作为现有单一论证贡献函数的泛化，建立相应的原则框架进行分析，并引入针对集合函数的新原则来关注论证间的交互特性。

Result: 开发了能够量化论证集合贡献的函数框架，提供了原则性分析方法，并通过推荐系统应用场景展示了不同集合贡献函数的原则表现。

Conclusion: 集合贡献函数为量化论证集合的整体影响提供了系统方法，扩展了论证贡献分析的理论基础，具有实际应用价值。
...
TL;DR: KAMAC是一个知识驱动的自适应多智能体协作框架，通过动态组建专家团队来提升医疗诊断决策能力，在复杂临床场景中显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 现有的多智能体协作框架使用静态预分配角色，限制了适应性和动态知识整合能力，无法满足复杂医疗诊断场景的需求。

Method: 提出KAMAC框架，从初始专家智能体开始，通过知识驱动讨论识别知识缺口，动态招募额外专家，支持灵活可扩展的协作，最终通过审查更新的智能体评论做出决策。

Result: 在两个真实医疗基准测试中，KAMAC显著优于单智能体和先进多智能体方法，特别是在需要动态跨专业知识的复杂临床场景（如癌症预后）中表现突出。

Conclusion: KAMAC框架通过动态团队组建和知识驱动的协作机制，有效解决了医疗决策中多专业知识整合的挑战，为复杂临床场景提供了更优的解决方案。
...
TL;DR: 本研究探讨了使用生成式AI对研究生在线课程中的同伴评审进行机器生成的元评审，以支持形成性评估。研究发现AI反馈能够近似有效人类反馈的关键修辞和关系特征。


<details>
  <summary>Details</summary>
Motivation: 探索生成式AI在支持形成性评估方面的潜力，特别是在同伴评审过程中提供机器生成的元评审，以提升反馈质量和学习效果。

Method: 基于系统功能语言学和评价理论，分析了120个元评审，考察生成式AI反馈在概念、人际和文本维度上的意义构建方式。

Result: 生成式AI反馈能够平衡表扬和建设性批评，符合评分标准期望，并采用结构化呈现方式突出学生主体性，近似有效人类反馈的关键特征。

Conclusion: AI元反馈具有搭建反馈素养支架和增强学习者参与同伴评审的潜力，能够提供清晰的指导性同时保持支持性立场。
...
TL;DR: 本文强调可解释AI(XAI)在海上领域的重要性，提出针对海事专业人员的信任、可用性和可解释性感知调查，以指导开发面向海员需求的用户中心XAI系统。


<details>
  <summary>Details</summary>
Motivation: 随着自主技术日益影响海上作业，理解AI系统决策原因与决策内容同等重要。在复杂动态的海上环境中，对AI的信任不仅取决于性能，还依赖于透明度和可解释性。

Method: 提出针对海事领域的专门调查问卷，旨在捕捉海事专业人员对信任、可用性和可解释性的感知，支持以用户为中心的XAI集成。

Result: 通过领域特定的调查方法，能够更好地理解海事专业人员对XAI系统的需求，为开发量身定制的可解释AI系统提供指导。

Conclusion: 可解释AI是海上领域有效人机协作的基础，用户中心的XAI系统开发需要充分考虑海员和海事团队的具体需求，调查方法是实现这一目标的重要工具。
...
TL;DR: 提出MACA强化学习框架，通过多智能体辩论的多数/少数结果来训练语言模型选择一致的推理路径，显著提升模型的自一致性和推理能力


<details>
  <summary>Details</summary>
Motivation: 语言模型存在推理不一致问题，对相同提示生成矛盾回答。现有推理时方法无法解决根本问题，即模型难以可靠选择导致一致结果的推理路径

Method: 引入多智能体共识对齐(MACA)强化学习框架，通过多智能体辩论产生多数/少数结果，训练模型偏好与其内部共识一致的推理轨迹

Result: 在自一致性(+27.6% GSM8K)、单智能体推理(+23.7% MATH)、采样推理(+22.4% Pass@20 MATH)和多智能体集成决策(+42.7% MathQA)方面显著提升，在未见基准上也有强泛化能力

Conclusion: MACA实现了强大的自对齐能力，更可靠地释放语言模型的潜在推理潜力，使智能体能够自我教导变得更果断和简洁
...
TL;DR: 提出RLVR方法改进几何图像数据生成，通过强化学习和可验证奖励优化几何问题描述，提升多模态大语言模型的几何推理能力，并在非几何任务上也能带来性能提升
...
Method: 引入强化学习与可验证奖励(RLVR)的数据生成流程，基于50个基本几何关系合成几何图像，利用数学问题求解任务作为奖励信号来优化图像描述

Result: 在MathVista和MathVerse的非几何图像任务上获得2.8%-4.8%的准确率提升，在MMMU的艺术、设计、技术和工程任务上获得2.4%-3.9%的改进

Conclusion: RLVR方法能有效捕捉几何问题求解的关键特征，提高任务泛化能力，即使在分布外场景下也能增强多模态大语言模型的通用推理能力
